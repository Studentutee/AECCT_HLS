...
int main() {
    ...
    static ac_fixed<64, 32, true> y_buf[Y_LEN]; // Q3.5
    static ac_fixed<64, 32, true> out_buf[OUT_LEN]; // Q3.5
    ...
    for (int s = s_begin; s < s_end; ++s) {
        ...
        ac_channel<ac_fixed<64,32,true>>   y_in_ch;
        ...
        for (int i = 0; i < Y_LEN; ++i) { //不同型態矩陣不能直接 memcpy, 只能一個一個轉
            y_buf[i] = ac_fixed<64, 32, true, AC_RND_CONV, AC_SAT_SYM>(y_ptr[i]); //data quantization
        }
        ...
        int mis = compare_array_abs<double, ac_fixed<64,32,true> >( // Q3.5
            exp_ptr,
            out_buf,
            OUT_LEN,
            (double)ATOL,
