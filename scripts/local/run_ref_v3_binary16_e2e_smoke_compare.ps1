param(
    [string]$BuildDir = "build\\ref_v3_binary16_e2e_smoke_compare"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir "tb_refmodel_v3_binary16_e2e_smoke_compare.exe"
    ...
    $sources = @(
        "AECCT_ac_ref\\tb\\ref_v3\\tb_refmodel_v3_binary16_e2e_smoke_compare.cpp",
    ...
    $summaryLines = @()
    $summaryLines += (Select-String -Path $runLog -Pattern '^\[ref_v3_binary16_smoke\]').Line
    $summaryLines += (Select-String -Path $runLog -Pattern '^\[ref_v3_binary16_smoke_summary\]').Line
    $summaryLines += (Select-String -Path $runLog -Pattern '^(PASS|FAIL): tb_refmodel_v3_binary16_e2e_smoke_compare').Line
    $summaryLines | Set-Content -Path $summaryLog -Encoding UTF8

    if ($runExit -eq 0) {
        Write-Host "PASS: run_ref_v3_binary16_e2e_smoke_compare"
        exit 0
    }

    Write-Host "FAIL: run_ref_v3_binary16_e2e_smoke_compare (run_exit=$runExit)"
    exit $runExit
}
catch {
    Write-Host ("FAIL: run_ref_v3_binary16_e2e_smoke_compare ({0})" -f $_.Exception.Message)
    exit 1
}
finally {
    Pop-Location
