param(
    [string]$BuildDir = "build\p11ah\full_loop"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_full_loop_local_e2e_p11ah.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_full_loop_local_e2e_p11ah.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    Add-Content -Path $runLog -Value 'PASS: run_p11ah_full_loop_local_e2e' -Encoding UTF8

    @(
        'task: P00-011AH',
        'status: PASS',
        'banner: PASS: run_p11ah_full_loop_local_e2e',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11ah_full_loop_local_e2e'
    exit 0
}
catch {
    Write-Error $_
