param(
    [string]$BuildDir = "build\\p11w4m1\\qkscore_phase_entry_probe"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_w4m1_qkscore_phase_entry_probe.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_w4m1_qkscore_phase_entry_probe.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    $requiredPassLines = @(
        ...
        'PASS: tb_w4m1_qkscore_phase_entry_probe'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11w4m1_qkscore_phase_entry_probe' -Encoding UTF8

    @(
        'task: P00-W4-M1-QKSCORE-PHASE-ENTRY-PROBE',
        'status: PASS',
        'banner: PASS: run_p11w4m1_qkscore_phase_entry_probe',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11w4m1_qkscore_phase_entry_probe'
    exit 0
}
catch {
    Write-Error $_
