param(
    [string]$BuildDir = "build\ref_v3_tail_boundary_compare"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir "tb_ref_v3_tail_boundary_compare.exe"
    ...
    $sources = @(
        "AECCT_ac_ref\tb\ref_v3\tb_ref_v3_tail_boundary_compare.cpp",
    ...
    Write-Host "BUILD_OK: tb_ref_v3_tail_boundary_compare"

    $runExit = Invoke-ExeRun -ExePath (Resolve-Path $exePath).Path -RunLog $runLog

    $summaryLines = @()
    $summaryLines += (Select-String -Path $runLog -Pattern '^\[ref_v3_tail_boundary_compare\]').Line
    $summaryLines += (Select-String -Path $runLog -Pattern '^\[ref_v3_tail_boundary_compare_summary\]').Line
    $summaryLines += (Select-String -Path $runLog -Pattern '^(PASS|FAIL): tb_ref_v3_tail_boundary_compare').Line
    if ($runExit -eq 0) {
        $summaryLines += "RUN_OK: tb_ref_v3_tail_boundary_compare"
    } else {
        $summaryLines += "RUN_FAIL: tb_ref_v3_tail_boundary_compare (run_exit=$runExit)"
    }
    $summaryLines | Set-Content -Path $summaryLog -Encoding UTF8

    if ($runExit -eq 0) {
        Write-Host "RUN_OK: tb_ref_v3_tail_boundary_compare"
        Write-Host "PASS: run_ref_v3_tail_boundary_compare"
        exit 0
    }

    Write-Host "RUN_FAIL: tb_ref_v3_tail_boundary_compare (run_exit=$runExit)"
    Write-Host "FAIL: run_ref_v3_tail_boundary_compare (run_exit=$runExit)"
    exit $runExit
}
catch {
    Write-Host ("FAIL: run_ref_v3_tail_boundary_compare ({0})" -f $_.Exception.Message)
    exit 1
}
finally {
    Pop-Location
