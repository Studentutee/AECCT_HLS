param(
    [string]$BuildDir = "build\\p11w4b9\\qkscore_longspan_bridge"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_w4b9_qkscore_longspan_bridge.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_w4b9_qkscore_longspan_bridge.cpp' -ExeOut $exePath -LogOut $buildLog
    Invoke-ExeRun -ExePath $exePath -LogOut $runLog

    $requiredPassLines = @(
        'W4B9_QKSCORE_LONGSPAN_BRIDGE_VISIBLE PASS',
        'W4B9_QKSCORE_LONGSPAN_OWNERSHIP_CHECK PASS',
        'W4B9_QKSCORE_LONGSPAN_EXPECTED_COMPARE PASS',
        'W4B9_QKSCORE_LONGSPAN_LEGACY_COMPARE PASS',
        'W4B9_QKSCORE_LONGSPAN_NO_SPURIOUS_TOUCH PASS',
        'W4B9_QKSCORE_LONGSPAN_MISMATCH_REJECT PASS',
        'W4B9_QKSCORE_LONGSPAN_MULTI_PATH_ANTI_FALLBACK PASS',
        'W4B9_QKSCORE_LONGSPAN_CONSUME_COUNT_EXACT PASS',
        'PASS: tb_w4b9_qkscore_longspan_bridge'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11w4b9_qkscore_longspan_bridge' -Encoding UTF8

    @(
        'task: P00-W4-B9-QKSCORE-LONGSPAN-BRIDGE',
        'status: PASS',
        'banner: PASS: run_p11w4b9_qkscore_longspan_bridge',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11w4b9_qkscore_longspan_bridge'
    exit 0
}
catch {
    Write-Error $_
