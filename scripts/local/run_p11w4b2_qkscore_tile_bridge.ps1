param(
    [string]$BuildDir = "build\\p11w4b2\\qkscore_tile_bridge"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_w4b2_qkscore_tile_bridge.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_w4b2_qkscore_tile_bridge.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    $requiredPassLines = @(
        ...
        'PASS: tb_w4b2_qkscore_tile_bridge'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11w4b2_qkscore_tile_bridge' -Encoding UTF8

    @(
        'task: P00-W4-B2-QKSCORE-TILE-BRIDGE',
        'status: PASS',
        'banner: PASS: run_p11w4b2_qkscore_tile_bridge',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11w4b2_qkscore_tile_bridge'
    exit 0
}
catch {
    Write-Error $_
