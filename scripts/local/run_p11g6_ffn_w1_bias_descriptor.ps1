param(
    [string]$BuildDir = "build\\p11g6\\ffn_w1_bias_descriptor"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_g6_ffn_w1_bias_descriptor_p11g6a.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_g6_ffn_w1_bias_descriptor_p11g6a.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    $requiredPassLines = @(
        ...
        'PASS: tb_g6_ffn_w1_bias_descriptor_p11g6a'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11g6_ffn_w1_bias_descriptor' -Encoding UTF8

    @(
        'task: P00-G6-FFN-SUBWAVE-A-W1-BIAS',
        'status: PASS',
        'banner: PASS: run_p11g6_ffn_w1_bias_descriptor',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11g6_ffn_w1_bias_descriptor'
    exit 0
}
catch {
    Write-Error $_
