...
try {
    ...
    Invoke-Step -StepName "run_p11ak_attnout_finalx_bridge" -Command "powershell" -StepArgs @(
        ...
        "-BuildDir", (Join-Path $BuildDir "p11ak")
    )

    Invoke-Step -StepName "run_p11aj_full_flow_batch" -Command "powershell" -StepArgs @(
        "-NoProfile", "-ExecutionPolicy", "Bypass",
        "-File", "scripts/local/run_p11aj_full_flow_batch.ps1",
        "-BuildDir", (Join-Path $BuildDir "p11aj_regression")
    )
    ...
    @(
        ...
        "PASS: run_p11ak_full_flow_batch"
    ) | Set-Content -Path $summaryPath -Encoding UTF8

    Write-Host "PASS: run_p11ak_full_flow_batch"
    exit 0
}
catch {
    Write-Error $_
