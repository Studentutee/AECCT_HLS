param(
    [string]$BuildDir = "build\\lid0_attention_multilayer_acceptance"
)
...
try {
    ...
    @(
        "task: lid0_attention_multilayer_acceptance",
        "status: PASS",
        "banner: PASS: run_lid0_attention_multilayer_acceptance",
        "scope: local-only",
    ...
    Write-Host "PASS: run_lid0_attention_multilayer_acceptance"
    exit 0
}
catch {
    Write-Error $_
