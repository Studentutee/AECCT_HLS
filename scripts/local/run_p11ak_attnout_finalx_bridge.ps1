param(
    [string]$BuildDir = "build\p11ak\p11ak"
)
...
try {
    ...
    $requiredPassLines = @(
        ...
        'BRIDGE_ATTNOUT_TO_FINALX_DIRECT_CONSUMPTION PASS',
        ...
        'PASS: tb_attnout_finalx_bridge_p11ak'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11ak_attnout_finalx_bridge' -Encoding UTF8

    @(
        'task: P00-011AK',
        'status: PASS',
        'banner: PASS: run_p11ak_attnout_finalx_bridge',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11ak_attnout_finalx_bridge'
    exit 0
}
catch {
    Write-Error $_
