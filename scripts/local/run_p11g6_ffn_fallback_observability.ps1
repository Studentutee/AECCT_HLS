param(
    [string]$BuildDir = "build\\p11g6\\ffn_fallback_observability"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_g6_ffn_fallback_observability_p11g6b.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_g6_ffn_fallback_observability_p11g6b.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    Add-Content -Path $runLog -Value 'PASS: run_p11g6_ffn_fallback_observability' -Encoding UTF8

    @(
        'task: P00-G6-FFN-SUBWAVE-B-OBS',
        'status: PASS',
        'banner: PASS: run_p11g6_ffn_fallback_observability',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11g6_ffn_fallback_observability'
    exit 0
}
catch {
    Write-Error $_
