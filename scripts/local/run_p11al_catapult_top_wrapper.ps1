param(
    [string]$BuildDir = "build\p11al\p11al"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_top_managed_catapult_wrapper_p11al.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_top_managed_catapult_wrapper_p11al.cpp' -ExeOut $exePath -LogOut $buildLog
    Invoke-ExeRun -ExePath $exePath -LogOut $runLog

    $requiredPassLines = @(
        'CATAPULT_TOP_WRAPPER_INTERFACE_SHAPE PASS',
        'TOP_WRAPPER_MAINLINE_PATH_TAKEN PASS',
        'TOP_WRAPPER_FALLBACK_NOT_TAKEN PASS',
        'TOP_WRAPPER_ATTN_OUT_EXPECTED_COMPARE PASS',
        'TOP_WRAPPER_FINAL_X_EXPECTED_COMPARE PASS',
        'fallback_taken = false',
        'PASS: tb_top_managed_catapult_wrapper_p11al'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11al_catapult_top_wrapper' -Encoding UTF8

    @(
        'task: P00-011AL',
        'status: PASS',
        'banner: PASS: run_p11al_catapult_top_wrapper',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11al_catapult_top_wrapper'
    exit 0
}
catch {
    Write-Error $_
