param(
    [string]$BuildDir = "build\p11ae_impl\p11ae"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_qk_score_impl_p11ae.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_qk_score_impl_p11ae.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    Add-Content -Path $runLog -Value 'PASS: run_p11ae_impl_qk_score' -Encoding UTF8

    @(
        'task: P00-011AE',
        'status: PASS',
        'banner: PASS: run_p11ae_impl_qk_score',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11ae_impl_qk_score'
    exit 0
}
catch {
    Write-Error $_
