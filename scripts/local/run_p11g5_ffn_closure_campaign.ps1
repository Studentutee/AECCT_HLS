param(
    [string]$BuildDir = "build\\p11g5\\ffn_closure_campaign"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_g5_ffn_closure_campaign_p11g5fc.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_g5_ffn_closure_campaign_p11g5fc.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    $requiredPassLines = @(
        ...
        'PASS: tb_g5_ffn_closure_campaign_p11g5fc'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11g5_ffn_closure_campaign' -Encoding UTF8

    @(
        'task: P00-G5-FFN-CLOSURE-CAMPAIGN',
        'status: PASS',
        'banner: PASS: run_p11g5_ffn_closure_campaign',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11g5_ffn_closure_campaign'
    exit 0
}
catch {
    Write-Error $_
