param(
    [string]$BuildDir = "build\\p11g5\\wave2_preproc_payload_migration"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_g5_wave2_preproc_payload_migration_p11g5w2.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_g5_wave2_preproc_payload_migration_p11g5w2.cpp' -ExeOut $exePath -LogOut $buildLog
    Invoke-ExeRun -ExePath $exePath -LogOut $runLog

    $requiredPassLines = @(
        'G5W2_PREPROC_TOPFED_INPUT_PATH PASS',
        'PASS: tb_g5_wave2_preproc_payload_migration_p11g5w2'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11g5_wave2_preproc_payload_migration' -Encoding UTF8

    @(
        'task: P00-G5-WAVE2-PREPROC-PAYLOAD-MIGRATION',
        'status: PASS',
        'banner: PASS: run_p11g5_wave2_preproc_payload_migration',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11g5_wave2_preproc_payload_migration'
    exit 0
}
catch {
    Write-Error $_
