param(
    [string]$BuildDir = "build\\p11g4f\\commit_diagnostics_negative"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_g4f_commit_diagnostics_negative_p11g4f.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_g4f_commit_diagnostics_negative_p11g4f.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    $requiredPassLines = @(
        ...
        'G4F_ACCEPTANCE_DETERMINISTIC PASS',
        'PASS: tb_g4f_commit_diagnostics_negative_p11g4f'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11g4f_commit_diagnostics_negative' -Encoding UTF8

    @(
        'task: P00-G4F-COMMIT-DIAG-NEG',
        'status: PASS',
        'banner: PASS: run_p11g4f_commit_diagnostics_negative',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11g4f_commit_diagnostics_negative'
    exit 0
}
catch {
    Write-Error $_
