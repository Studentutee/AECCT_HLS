...
try {
    ...
    Invoke-Step -StepName "run_p11am_catapult_compile_surface" -Command "powershell" -StepArgs @(
        "-NoProfile", "-ExecutionPolicy", "Bypass",
        "-File", "scripts/local/run_p11am_catapult_compile_surface.ps1",
        "-BuildDir", (Join-Path $BuildDir "p11am_main")
    )
    ...
    @(
        ...
        "PASS: run_p11am_catapult_surface_batch"
    ) | Set-Content -Path $summaryPath -Encoding UTF8

    Write-Host "PASS: run_p11am_catapult_surface_batch"
    exit 0
}
catch {
    Write-Error $_
