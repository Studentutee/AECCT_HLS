param(
    [string]$BuildDir = "build\\p11g5\\wave3_ffn_payload_migration"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_g5_wave3_ffn_payload_migration_p11g5w3.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_g5_wave3_ffn_payload_migration_p11g5w3.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    $requiredPassLines = @(
        ...
        'PASS: tb_g5_wave3_ffn_payload_migration_p11g5w3'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11g5_wave3_ffn_payload_migration' -Encoding UTF8

    @(
        'task: P00-G5-WAVE3-FFN-PAYLOAD-MIGRATION',
        'status: PASS',
        'banner: PASS: run_p11g5_wave3_ffn_payload_migration',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11g5_wave3_ffn_payload_migration'
    exit 0
}
catch {
    Write-Error $_
