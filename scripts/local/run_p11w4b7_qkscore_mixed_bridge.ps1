param(
    [string]$BuildDir = "build\\p11w4b7\\qkscore_mixed_bridge"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_w4b7_qkscore_mixed_bridge.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_w4b7_qkscore_mixed_bridge.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    $requiredPassLines = @(
        ...
        'PASS: tb_w4b7_qkscore_mixed_bridge'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11w4b7_qkscore_mixed_bridge' -Encoding UTF8

    @(
        'task: P00-W4-B7-QKSCORE-MIXED-BRIDGE',
        'status: PASS',
        'banner: PASS: run_p11w4b7_qkscore_mixed_bridge',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11w4b7_qkscore_mixed_bridge'
    exit 0
}
catch {
    Write-Error $_
