param(
    [string]$BuildDir = "build\ref_v3_binary16_capability_compare"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir "tb_refmodel_v3_binary16_capability_compare.exe"
    ...
    $sources = @(
        "AECCT_ac_ref\tb\ref_v3\tb_refmodel_v3_binary16_capability_compare.cpp",
        "AECCT_ac_ref\src\RefModel.cpp",
        "AECCT_ac_ref\src\RefModelOptimized.cpp",
        "AECCT_ac_ref\src\ref_v3\RefModel_v3.cpp",
    ...
    $summaryLines += "BUILD_OK: tb_refmodel_v3_binary16_capability_compare"

    $runExit = Invoke-ExeRun -ExePath (Resolve-Path $exePath).Path -RunLog $runLog
    if ($runExit -eq 0) {
        $summaryLines += "RUN_OK: tb_refmodel_v3_binary16_capability_compare"
    } else {
        $summaryLines += ("RUN_FAIL: tb_refmodel_v3_binary16_capability_compare (exit={0})" -f $runExit)
    }

    $summaryLines += (Select-String -Path $runLog -Pattern '^\[ref_v3_binary16_capability_compare\]').Line
    $summaryLines += (Select-String -Path $runLog -Pattern '^\[ref_v3_binary16_capability_compare_summary\]').Line
    $summaryLines += (Select-String -Path $runLog -Pattern '^(PASS|FAIL): tb_refmodel_v3_binary16_capability_compare').Line
    $summaryLines | Set-Content -Path $summaryLog -Encoding UTF8

    if ($runExit -eq 0) {
        Write-Host "PASS: run_ref_v3_binary16_capability_compare"
        exit 0
    }

    Write-Host "FAIL: run_ref_v3_binary16_capability_compare (run_exit=$runExit)"
    exit $runExit
}
catch {
    Write-Host ("FAIL: run_ref_v3_binary16_capability_compare ({0})" -f $_.Exception.Message)
    exit 1
}
finally {
    Pop-Location
