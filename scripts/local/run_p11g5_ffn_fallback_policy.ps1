param(
    [string]$BuildDir = "build\\p11g5\\ffn_fallback_policy"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_g5_ffn_fallback_policy_p11g5fp.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_g5_ffn_fallback_policy_p11g5fp.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    $requiredPassLines = @(
        ...
        'PASS: tb_g5_ffn_fallback_policy_p11g5fp'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11g5_ffn_fallback_policy' -Encoding UTF8

    @(
        'task: P00-G5-FFN-FALLBACK-POLICY',
        'status: PASS',
        'banner: PASS: run_p11g5_ffn_fallback_policy',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11g5_ffn_fallback_policy'
    exit 0
}
catch {
    Write-Error $_
