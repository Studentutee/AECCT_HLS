param(
    [string]$BuildDir = "build\\p11g4\\infer_ingest_preflight_negative"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_infer_ingest_preflight_negative_p11g4.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_infer_ingest_preflight_negative_p11g4.cpp' -ExeOut $exePath -LogOut $buildLog
    Invoke-ExeRun -ExePath $exePath -LogOut $runLog

    $requiredPassLines = @(
        'PREFLIGHT_VALID_CONTRACT PASS',
        'PREFLIGHT_INVALID_BASE_REJECT PASS',
        'PREFLIGHT_INVALID_SPAN_REJECT PASS',
        'PREFLIGHT_VALID_EDGE_ACCEPT PASS',
        'PREFLIGHT_ERR_MEM_RANGE_GUARD_BEHAVIOR PASS',
        'PASS: tb_infer_ingest_preflight_negative_p11g4'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11g4_infer_ingest_preflight_negative' -Encoding UTF8

    @(
        'task: P00-G4-PREFLIGHT-NEG',
        'status: PASS',
        'banner: PASS: run_p11g4_infer_ingest_preflight_negative',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11g4_infer_ingest_preflight_negative'
    exit 0
}
catch {
    Write-Error $_
