param(
    [string]$BuildDir = "build\\p11w4b3\\qkscore_bridge"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_w4b3_qkscore_bridge.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_w4b3_qkscore_bridge.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    $requiredPassLines = @(
        ...
        'PASS: tb_w4b3_qkscore_bridge'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11w4b3_qkscore_bridge' -Encoding UTF8

    @(
        'task: P00-W4-B3-QKSCORE-BRIDGE',
        'status: PASS',
        'banner: PASS: run_p11w4b3_qkscore_bridge',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11w4b3_qkscore_bridge'
    exit 0
}
catch {
    Write-Error $_
