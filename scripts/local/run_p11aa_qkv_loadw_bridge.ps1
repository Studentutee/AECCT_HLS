param(
    [string]$BuildDir = "build\p11aa\p11aa"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_qkv_formal_loadw_bridge_p11aa.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_qkv_formal_loadw_bridge_p11aa.cpp' -ExeOut $exePath -LogOut $buildLog
    Invoke-ExeRun -ExePath $exePath -LogOut $runLog

    Require-PassString -LogPath $runLog -Needle 'PASS: tb_qkv_formal_loadw_bridge_p11aa'

    Add-Content -Path $runLog -Value 'PASS: run_p11aa_qkv_loadw_bridge' -Encoding UTF8

    @(
        'task: P00-011AA',
        'status: PASS',
        'banner: PASS: run_p11aa_qkv_loadw_bridge',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11aa_qkv_loadw_bridge'
    exit 0
}
catch {
    Write-Error $_
