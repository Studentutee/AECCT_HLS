param(
    [string]$BuildDir = "build\\p11g4g\\accept_commit_record"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_g4g_accept_commit_record_p11g4g.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_g4g_accept_commit_record_p11g4g.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    $requiredPassLines = @(
        ...
        'PASS: tb_g4g_accept_commit_record_p11g4g'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11g4g_accept_commit_record' -Encoding UTF8

    @(
        'task: P00-G4G-ACCEPT-COMMIT-RECORD',
        'status: PASS',
        'banner: PASS: run_p11g4g_accept_commit_record',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11g4g_accept_commit_record'
    exit 0
}
catch {
    Write-Error $_
