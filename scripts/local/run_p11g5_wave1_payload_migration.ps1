param(
    [string]$BuildDir = "build\\p11g5\\wave1_payload_migration"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_g5_wave1_payload_migration_p11g5w1.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_g5_wave1_payload_migration_p11g5w1.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    $requiredPassLines = @(
        ...
        'PASS: tb_g5_wave1_payload_migration_p11g5w1'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11g5_wave1_payload_migration' -Encoding UTF8

    @(
        'task: P00-G5-WAVE1-PAYLOAD-MIGRATION',
        'status: PASS',
        'banner: PASS: run_p11g5_wave1_payload_migration',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11g5_wave1_payload_migration'
    exit 0
}
catch {
    Write-Error $_
