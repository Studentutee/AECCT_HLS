param(
    [string]$BuildDir = "build\\p11g5\\ffn_w1_fallback_policy"
)
...
try {
    ...
    Add-Content -Path $runLog -Value 'PASS: run_p11g5_ffn_w1_fallback_policy' -Encoding UTF8

    @(
        'task: P00-G5-FFN-W1-FALLBACK-POLICY',
        'status: PASS',
        'banner: PASS: run_p11g5_ffn_w1_fallback_policy',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11g5_ffn_w1_fallback_policy'
    exit 0
}
catch {
    Write-Error $_
