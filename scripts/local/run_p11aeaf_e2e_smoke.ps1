param(
    [string]$BuildDir = "build\p11aeaf_e2e\p11aeaf"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_e2e_correction_smoke_p11aeaf.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_e2e_correction_smoke_p11aeaf.cpp' -ExeOut $exePath -LogOut $buildLog
    Invoke-ExeRun -ExePath $exePath -LogOut $runLog

    Require-PassString -LogPath $runLog -Needle 'PASS: tb_e2e_correction_smoke_p11aeaf'

    Add-Content -Path $runLog -Value 'PASS: run_p11aeaf_e2e_smoke' -Encoding UTF8

    @(
        'task: P00-011AEAF-E2E',
        'status: PASS',
        'banner: PASS: run_p11aeaf_e2e_smoke',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11aeaf_e2e_smoke'
    exit 0
}
catch {
    Write-Error $_
