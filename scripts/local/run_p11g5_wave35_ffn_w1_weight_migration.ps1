param(
    [string]$BuildDir = "build\\p11g5\\wave35_ffn_w1_weight_migration"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_g5_wave35_ffn_w1_weight_migration_p11g5w35.exe'
    ...
    Invoke-ClBuild -Source 'tb\tb_g5_wave35_ffn_w1_weight_migration_p11g5w35.cpp' -ExeOut $exePath -LogOut $buildLog
    ...
    $requiredPassLines = @(
        ...
        'PASS: tb_g5_wave35_ffn_w1_weight_migration_p11g5w35'
    )
    foreach ($line in $requiredPassLines) {
        Require-PassString -LogPath $runLog -Needle $line
    }

    Add-Content -Path $runLog -Value 'PASS: run_p11g5_wave35_ffn_w1_weight_migration' -Encoding UTF8

    @(
        'task: P00-G5-WAVE35-FFN-W1-WEIGHT-MIGRATION',
        'status: PASS',
        'banner: PASS: run_p11g5_wave35_ffn_w1_weight_migration',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11g5_wave35_ffn_w1_weight_migration'
    exit 0
}
catch {
    Write-Error $_
