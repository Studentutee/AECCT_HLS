param(
    [string]$BuildDir = "build\p11ac\p11ac"
)
...
try {
    ...
    $exePath = Join-Path $BuildDir 'tb_kv_build_stream_stage_p11ac.exe'
    ...
    @(
        'task: P00-011AC',
        'status: PASS',
        'banner: PASS: run_p11ac_phasea_top_managed',
        'scope: local-only',
    ...
    Write-Host 'PASS: run_p11ac_phasea_top_managed'
    exit 0
}
catch {
    Write-Error $_
