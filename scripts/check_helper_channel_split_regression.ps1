...
Require-Regex -Text $acText -Pattern '(?m)^\s*typedef\s+ac_channel<AttnTopManagedWorkPacket>\s+attn_k_work_pkt_ch_t\s*;' -Reason "AC split typedef (k work out channel) missing"
Require-Regex -Text $acText -Pattern '(?m)^\s*typedef\s+ac_channel<AttnTopManagedWorkPacket>\s+attn_v_work_pkt_ch_t\s*;' -Reason "AC split typedef (v work out channel) missing"
Require-Regex -Text $acText -Pattern 'attn_block_phasea_kv_consume_emit_token_work_tiles\s*\(\s*attn_x_work_pkt_ch_t&\s*x_ch\s*,\s*attn_wk_work_pkt_ch_t&\s*wk_ch\s*,\s*attn_wv_work_pkt_ch_t&\s*wv_ch\s*,\s*attn_k_work_pkt_ch_t&\s*k_ch\s*,\s*attn_v_work_pkt_ch_t&\s*v_ch\s*,' -Reason "AC work-tile consume signature must keep x/wk/wv in and k/v out split channels"
Require-Regex -Text $acText -Pattern 'attn_top_writeback_phasea_kv_work_tile\s*\(\s*SramView&\s+sram[^\)]*attn_k_work_pkt_ch_t&\s*k_ch\s*,\s*attn_v_work_pkt_ch_t&\s*v_ch\s*\)' -Reason "AC work-tile writeback signature must keep k/v split channels"
...
Require-Regex -Text $acText -Pattern '\bk_ch\.write\s*\(\s*k_pkt\s*\)' -Reason "AC work-tile path must write K to split k_ch"
Require-Regex -Text $acText -Pattern '\bv_ch\.write\s*\(\s*v_pkt\s*\)' -Reason "AC work-tile path must write V to split v_ch"
Require-Regex -Text $acText -Pattern '\bk_ch\.nb_read\s*\(\s*k_pkt\s*\)' -Reason "AC work-tile writeback must read K from split k_ch"
Require-Regex -Text $acText -Pattern '\bv_ch\.nb_read\s*\(\s*v_pkt\s*\)' -Reason "AC work-tile writeback must read V from split v_ch"
...
Forbid-Regex -Text $acText -Pattern 'attn_block_phasea_kv_consume_emit_token_work_tiles\s*\([^\)]*attn_work_pkt_ch_t&\s*out_ch' -Reason "AC regression detected: shared out_ch consume signature restored"
Forbid-Regex -Text $acText -Pattern 'attn_top_writeback_phasea_kv_work_tile\s*\([^\)]*attn_work_pkt_ch_t&\s*out_ch' -Reason "AC regression detected: shared out_ch writeback signature restored"
