param(
    [string]$RepoRoot = ".",
    [string]$OutDir = "build\p11ad_prep",
...
$logPath = Join-Path $outDirAbs "check_p11ad_prep_surface.log"
$summaryPath = Join-Path $outDirAbs "check_p11ad_prep_surface_summary.txt"

function Fail-Check([string]$Reason) {
    "FAIL: check_p11ad_prep_surface" | Tee-Object -FilePath $logPath -Append
...
$tbText = Get-Content -Path (Join-Path $repo "tb/tb_q_path_scaffold_p11ad_prep.cpp") -Raw
...
if ($Phase -eq "post") {
    $reportText = Get-Content -Path (Join-Path $repo "docs/milestones/P00-011AD-prep_report.md") -Raw
    if ($reportText -notmatch 'local-only') { Fail-Check "report missing local-only wording" }
}

"PASS: check_p11ad_prep_surface" | Tee-Object -FilePath $logPath -Append
