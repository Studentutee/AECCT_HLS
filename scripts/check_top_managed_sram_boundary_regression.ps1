...
Require-Regex -Text $ffnText -Pattern '(?ms)FFNLayer0CoreWindow\s*\([\s\S]*?u32_t\s+layer_id\s*=\s*\(u32_t\)0\s*,\s*const\s+u32_t\*\s+topfed_x_words\s*=\s*0\s*,\s*const\s+u32_t\*\s+topfed_w1_weight_words\s*=\s*0\s*,\s*u32_t\s+topfed_w1_weight_words_valid\s*=\s*0\s*,\s*const\s+u32_t\*\s+topfed_w2_input_words\s*=\s*0\s*,\s*u32_t\s+topfed_w2_input_words_valid\s*=\s*0\s*,\s*const\s+u32_t\*\s+topfed_w2_weight_words\s*=\s*0\s*,\s*u32_t\s+topfed_w2_weight_words_valid\s*=\s*0\s*,\s*const\s+u32_t\*\s+topfed_w2_bias_words\s*=\s*0\s*,\s*u32_t\s+topfed_w2_bias_words_valid\s*=\s*0' -Reason "FFNLayer0CoreWindow top-fed payload descriptor arguments missing"
...
Require-Regex -Text $layerText -Pattern '(?ms)FFNLayer0TopManagedWindowBridge<\s*FFN_STAGE_W1\s*>\s*\([\s\S]*?topfed_ffn_x_words\s*,[\s\S]*?topfed_ffn_w1_words\s*,[\s\S]*?\(u32_t\)w1_weight_words' -Reason "TransformerLayerTopManagedAttnBridge must dispatch FFN top-fed x+w1 W1 stage payload"
...
Require-Regex -Text $layerText -Pattern '(?ms)FFNLayer0TopManagedWindowBridge<\s*FFN_STAGE_W2\s*>\s*\([\s\S]*?topfed_ffn_w2_input_words\s*,[\s\S]*?\(u32_t\)w2_input_words[\s\S]*?topfed_ffn_w2_words\s*,[\s\S]*?\(u32_t\)w2_weight_words[\s\S]*?topfed_ffn_w2_bias_words\s*,[\s\S]*?\(u32_t\)w2_bias_words' -Reason "TransformerLayerTopManagedAttnBridge must dispatch FFN top-fed W2 input/weight/bias payload"
Require-Regex -Text $layerText -Pattern '(?ms)FFNLayer0<\s*FFN_STAGE_W1\s*>\s*\([\s\S]*?topfed_ffn_x_words\s*,[\s\S]*?topfed_ffn_w1_words\s*,[\s\S]*?\(u32_t\)w1_weight_words' -Reason "TransformerLayer pointer path must dispatch FFN top-fed x+w1 W1 stage payload"
