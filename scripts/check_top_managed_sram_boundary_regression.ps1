...
Require-Regex -Text $topText -Pattern '(?ms)run_p11ae_layer0_top_managed_qk_score\s*\([\s\S]*?score_tile_bridge_base_word[\s\S]*?score_tile_bridge_words[\s\S]*?score_tile_bridge_words_valid[\s\S]*?score_tile_bridge_key_begin[\s\S]*?score_tile_bridge_visible[\s\S]*?score_tile_bridge_owner_ok[\s\S]*?score_tile_bridge_consumed[\s\S]*?score_tile_bridge_compare_ok[\s\S]*?score_tile_bridge_head_idx' -Reason "Top run_p11ae helper missing W4-B3 selectable-head score-tile bridge arguments"
Require-Regex -Text $topText -Pattern '(?ms)run_p11ae_layer0_top_managed_qk_score[\s\S]*?attn_phaseb_top_managed_qk_score_mainline\s*\([\s\S]*?score_tile_bridge_base_word[\s\S]*?score_tile_bridge_words[\s\S]*?score_tile_bridge_words_valid[\s\S]*?score_tile_bridge_key_begin[\s\S]*?score_tile_bridge_visible[\s\S]*?score_tile_bridge_owner_ok[\s\S]*?score_tile_bridge_consumed[\s\S]*?score_tile_bridge_compare_ok[\s\S]*?score_tile_bridge_head_idx' -Reason "Top run_p11ae helper must pass through W4-B3 selectable-head score-tile bridge arguments"
Require-Regex -Text $topText -Pattern '(?ms)run_p11ae_layer0_top_managed_qk_score\s*\([\s\S]*?score_tile_bridge_head_idx[\s\S]*?score_tile_bridge_family_case_count[\s\S]*?score_tile_bridge_family_base_words[\s\S]*?score_tile_bridge_family_words[\s\S]*?score_tile_bridge_family_words_valid[\s\S]*?score_tile_bridge_family_key_begin[\s\S]*?score_tile_bridge_family_head_idx[\s\S]*?score_tile_bridge_family_visible_count[\s\S]*?score_tile_bridge_family_owner_ok[\s\S]*?score_tile_bridge_family_consumed_count[\s\S]*?score_tile_bridge_family_compare_ok[\s\S]*?score_tile_bridge_family_case_mask' -Reason "Top run_p11ae helper missing W4-B5 family bridge arguments"
Require-Regex -Text $topText -Pattern '(?ms)run_p11ae_layer0_top_managed_qk_score[\s\S]*?attn_phaseb_top_managed_qk_score_mainline\s*\([\s\S]*?score_tile_bridge_head_idx[\s\S]*?score_tile_bridge_family_case_count[\s\S]*?score_tile_bridge_family_base_words[\s\S]*?score_tile_bridge_family_words[\s\S]*?score_tile_bridge_family_words_valid[\s\S]*?score_tile_bridge_family_key_begin[\s\S]*?score_tile_bridge_family_head_idx[\s\S]*?score_tile_bridge_family_visible_count[\s\S]*?score_tile_bridge_family_owner_ok[\s\S]*?score_tile_bridge_family_consumed_count[\s\S]*?score_tile_bridge_family_compare_ok[\s\S]*?score_tile_bridge_family_case_mask' -Reason "Top run_p11ae helper must pass through W4-B5 family bridge arguments"
...
Require-Regex -Text $qkScoreText -Pattern '(?ms)score_tile_bridge_head_idx_u32' -Reason "QK-score mainline missing W4-B3 bridge-head selector anchor"
...
Require-Regex -Text $qkScoreText -Pattern '(?ms)\(\s*h\s*==\s*score_tile_bridge_head_idx_u32\s*\)' -Reason "QK-score mainline missing W4-B3 non-head0 capable bridge selection anchor"
...
Require-Regex -Text $qkScoreText -Pattern '(?ms)score_tile_bridge_family_case_count' -Reason "QK-score mainline missing W4-B5 family case-count anchor"
Require-Regex -Text $qkScoreText -Pattern '(?ms)score_tile_bridge_family_enabled' -Reason "QK-score mainline missing W4-B5 family-enable anchor"
Require-Regex -Text $qkScoreText -Pattern '(?ms)kScoreTileBridgeFamilyMaxCases\s*=\s*8u' -Reason "QK-score mainline missing W4-B8 family capacity anchor (8 cases)"
Require-Regex -Text $qkScoreText -Pattern '(?ms)kScoreTileBridgeFamilyStrideWords\s*=\s*\(uint32_t\)ATTN_TOKEN_COUNT' -Reason "QK-score mainline missing W4-B9 token-span family stride anchor"
Require-Regex -Text $qkScoreText -Pattern '(?ms)score_tile_bridge_valid_words\s*>\s*\(uint32_t\)ATTN_TOKEN_COUNT' -Reason "QK-score mainline missing W4-B9 single bridge token-span guard anchor"
Require-Regex -Text $qkScoreText -Pattern '(?ms)valid\s*>\s*token_count\s*\|\|\s*valid\s*>\s*\(uint32_t\)ATTN_TOKEN_COUNT' -Reason "QK-score mainline missing W4-B9 family token-span guard anchor"
Require-Regex -Text $qkScoreText -Pattern '(?ms)ATTN_P11AE_SCORE_TILE_FAMILY_PRECHECK_LOOP' -Reason "QK-score mainline missing W4-B5 family precheck loop label"
Require-Regex -Text $qkScoreText -Pattern '(?ms)ATTN_P11AE_SCORE_TILE_FAMILY_CASE_LOOP' -Reason "QK-score mainline missing W4-B5 family select loop label"
Require-Regex -Text $qkScoreText -Pattern '(?ms)score_tile_bridge_family_case_idx' -Reason "QK-score mainline missing W4-B5 family selected-case anchor"
Require-Regex -Text $qkScoreText -Pattern '(?ms)score_tile_bridge_family_case_mask_u32' -Reason "QK-score mainline missing W4-B5 family case-mask observability anchor"
Require-Regex -Text $qkScoreText -Pattern '(?ms)score_tile_bridge_family_owner_ok' -Reason "QK-score mainline missing W4-B5 family ownership observability anchor"
Require-Regex -Text $qkScoreText -Pattern '(?ms)score_tile_bridge_family_compare_ok' -Reason "QK-score mainline missing W4-B5 family compare observability anchor"
Require-Regex -Text $qkScoreText -Pattern '(?ms)score_tile_bridge_family_consumed_count' -Reason "QK-score mainline missing W4-B5 family consumed observability anchor"
...
Require-Regex -Text $topText -Pattern '(?ms)run_p11af_layer0_top_managed_softmax_out\s*\([\s\S]*?phase_tile_bridge_compare_ok[\s\S]*?phase_tile_bridge_family_case_count[\s\S]*?phase_tile_bridge_family_v_base_words[\s\S]*?phase_tile_bridge_family_v_words[\s\S]*?phase_tile_bridge_family_v_words_valid[\s\S]*?phase_tile_bridge_family_d_tile_idx[\s\S]*?phase_tile_bridge_family_visible_count[\s\S]*?phase_tile_bridge_family_owner_ok[\s\S]*?phase_tile_bridge_family_consumed_count[\s\S]*?phase_tile_bridge_family_compare_ok[\s\S]*?phase_tile_bridge_family_case_mask' -Reason "Top run_p11af helper missing W4-C0 SoftmaxOut family bridge arguments"
Require-Regex -Text $topText -Pattern '(?ms)run_p11af_layer0_top_managed_softmax_out[\s\S]*?attn_phaseb_top_managed_softmax_out_mainline\s*\([\s\S]*?phase_tile_bridge_compare_ok[\s\S]*?phase_tile_bridge_family_case_count[\s\S]*?phase_tile_bridge_family_v_base_words[\s\S]*?phase_tile_bridge_family_v_words[\s\S]*?phase_tile_bridge_family_v_words_valid[\s\S]*?phase_tile_bridge_family_d_tile_idx[\s\S]*?phase_tile_bridge_family_visible_count[\s\S]*?phase_tile_bridge_family_owner_ok[\s\S]*?phase_tile_bridge_family_consumed_count[\s\S]*?phase_tile_bridge_family_compare_ok[\s\S]*?phase_tile_bridge_family_case_mask' -Reason "Top run_p11af helper must pass through W4-C0 SoftmaxOut family bridge arguments"
Require-Regex -Text $topText -Pattern '(?ms)run_p11af_layer0_top_managed_softmax_out\s*\([\s\S]*?phase_tile_bridge_family_case_mask[\s\S]*?phase_tile_bridge_family_head_idx[\s\S]*?phase_tile_bridge_family_key_token_begin[\s\S]*?phase_tile_bridge_family_key_token_count[\s\S]*?phase_tile_bridge_family_desc_visible_count[\s\S]*?phase_tile_bridge_family_desc_case_mask' -Reason "Top run_p11af helper missing W4-C1 head/token descriptor arguments"
Require-Regex -Text $topText -Pattern '(?ms)run_p11af_layer0_top_managed_softmax_out[\s\S]*?attn_phaseb_top_managed_softmax_out_mainline\s*\([\s\S]*?phase_tile_bridge_family_case_mask[\s\S]*?phase_tile_bridge_family_head_idx[\s\S]*?phase_tile_bridge_family_key_token_begin[\s\S]*?phase_tile_bridge_family_key_token_count[\s\S]*?phase_tile_bridge_family_desc_visible_count[\s\S]*?phase_tile_bridge_family_desc_case_mask' -Reason "Top run_p11af helper must pass through W4-C1 head/token descriptor arguments"
Require-Regex -Text $topText -Pattern '(?ms)run_p11af_layer0_top_managed_softmax_out\s*\([\s\S]*?phase_tile_bridge_family_desc_case_mask[\s\S]*?phase_tile_bridge_family_renorm_selected_count[\s\S]*?phase_tile_bridge_family_renorm_case_mask' -Reason "Top run_p11af helper missing W4-C3 renorm observability arguments"
Require-Regex -Text $topText -Pattern '(?ms)run_p11af_layer0_top_managed_softmax_out[\s\S]*?attn_phaseb_top_managed_softmax_out_mainline\s*\([\s\S]*?phase_tile_bridge_family_desc_case_mask[\s\S]*?phase_tile_bridge_family_renorm_selected_count[\s\S]*?phase_tile_bridge_family_renorm_case_mask' -Reason "Top run_p11af helper must pass through W4-C3 renorm observability arguments"
Require-Regex -Text $topText -Pattern '(?ms)run_p11af_layer0_top_managed_softmax_out\s*\([\s\S]*?phase_tile_bridge_family_renorm_case_mask[\s\S]*?phase_tile_bridge_family_writeback_selected_count[\s\S]*?phase_tile_bridge_family_writeback_case_mask[\s\S]*?phase_tile_bridge_family_writeback_touch_count' -Reason "Top run_p11af helper missing W4-C4 writeback observability arguments"
Require-Regex -Text $topText -Pattern '(?ms)run_p11af_layer0_top_managed_softmax_out[\s\S]*?attn_phaseb_top_managed_softmax_out_mainline\s*\([\s\S]*?phase_tile_bridge_family_renorm_case_mask[\s\S]*?phase_tile_bridge_family_writeback_selected_count[\s\S]*?phase_tile_bridge_family_writeback_case_mask[\s\S]*?phase_tile_bridge_family_writeback_touch_count' -Reason "Top run_p11af helper must pass through W4-C4 writeback observability arguments"
...
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_case_count' -Reason "SoftmaxOut mainline missing W4-C0 family case-count anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_enabled' -Reason "SoftmaxOut mainline missing W4-C0 family-enable anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)ATTN_P11AF_TILE_BRIDGE_FAMILY_PRECHECK_LOOP' -Reason "SoftmaxOut mainline missing W4-C0 family precheck loop label"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)ATTN_P11AF_TILE_BRIDGE_FAMILY_CASE_LOOP' -Reason "SoftmaxOut mainline missing W4-C0 family case select loop label"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)ATTN_P11AF_TILE_BRIDGE_FAMILY_COMPARE_LOOP' -Reason "SoftmaxOut mainline missing W4-C0 family compare loop label"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)ATTN_P11AF_TILE_BRIDGE_FAMILY_FINALIZE_LOOP' -Reason "SoftmaxOut mainline missing W4-C0 family finalize loop label"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_case_mask_u32' -Reason "SoftmaxOut mainline missing W4-C0 family case-mask observability anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_owner_ok' -Reason "SoftmaxOut mainline missing W4-C0 family ownership observability anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_compare_ok' -Reason "SoftmaxOut mainline missing W4-C0 family compare observability anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_consumed_count' -Reason "SoftmaxOut mainline missing W4-C0 family consumed observability anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_head_idx' -Reason "SoftmaxOut mainline missing W4-C1 family head descriptor anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_key_token_begin' -Reason "SoftmaxOut mainline missing W4-C1 family token-begin descriptor anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_key_token_count' -Reason "SoftmaxOut mainline missing W4-C1 family token-count descriptor anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_desc_visible_count' -Reason "SoftmaxOut mainline missing W4-C1 descriptor visibility observability anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_desc_case_mask' -Reason "SoftmaxOut mainline missing W4-C1 descriptor case-mask observability anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)ATTN_P11AF_TILE_BRIDGE_FAMILY_DESC_PROBE_LOOP' -Reason "SoftmaxOut mainline missing W4-C1 descriptor probe loop label"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)case_key_token_count\s*!=\s*1u' -Reason "SoftmaxOut mainline missing W4-C2 bounded single-token selector guard anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_later_case_count_u32' -Reason "SoftmaxOut mainline missing W4-C2 later-token bounded case-count anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)ATTN_P11AF_TILE_BRIDGE_FAMILY_ACC_CASE_LOOP' -Reason "SoftmaxOut mainline missing W4-C2 ACC family case loop label"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)ATTN_P11AF_TILE_BRIDGE_FAMILY_ACC_COMPARE_LOOP' -Reason "SoftmaxOut mainline missing W4-C2 ACC family compare loop label"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_renorm_selected_count' -Reason "SoftmaxOut mainline missing W4-C3 renorm selected-count observability anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_renorm_case_mask' -Reason "SoftmaxOut mainline missing W4-C3 renorm case-mask observability anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)ATTN_P11AF_TILE_BRIDGE_FAMILY_RENORM_CASE_LOOP' -Reason "SoftmaxOut mainline missing W4-C3 renorm family case loop label"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)ATTN_P11AF_TILE_BRIDGE_FAMILY_RENORM_COMPARE_LOOP' -Reason "SoftmaxOut mainline missing W4-C3 renorm family compare loop label"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_writeback_selected_count' -Reason "SoftmaxOut mainline missing W4-C4 writeback selected-count observability anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_writeback_case_mask' -Reason "SoftmaxOut mainline missing W4-C4 writeback case-mask observability anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_writeback_touch_count' -Reason "SoftmaxOut mainline missing W4-C4 writeback touch-count observability anchor"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)ATTN_P11AF_TILE_BRIDGE_FAMILY_WRITEBACK_CASE_LOOP' -Reason "SoftmaxOut mainline missing W4-C4 writeback family case loop label"
...
Require-Regex -Text $layerText -Pattern '(?ms)FFNLayer0TopManagedWindowBridge<\s*FFN_STAGE_W1\s*>\s*\([\s\S]*?(?:topfed_ffn_x_words|selected_topfed_ffn_x_words)\s*,[\s\S]*?(?:topfed_ffn_w1_words|selected_topfed_ffn_w1_words)\s*,[\s\S]*?(?:\(u32_t\)w1_weight_words|selected_topfed_ffn_w1_words_valid)[\s\S]*?\(u32_t\)FFN_POLICY_REQUIRE_W1_TOPFED[\s\S]*?(?:\(u32_t\)ffn_x_words|selected_topfed_ffn_x_words_valid)[\s\S]*?(?:topfed_ffn_w1_bias_words|selected_topfed_ffn_w1_bias_words)\s*,[\s\S]*?(?:\(u32_t\)w1_bias_words|selected_topfed_ffn_w1_bias_words_valid)' -Reason "TransformerLayerTopManagedAttnBridge must dispatch strict top-fed W1 policy with x and W1-bias descriptors"
...
Require-Regex -Text $layerText -Pattern '(?ms)FFNLayer0TopManagedWindowBridge<\s*FFN_STAGE_W2\s*>\s*\([\s\S]*?(?:topfed_ffn_w2_input_words|selected_topfed_ffn_w2_input_words)\s*,[\s\S]*?(?:\(u32_t\)w2_input_words|selected_topfed_ffn_w2_input_words_valid)[\s\S]*?(?:topfed_ffn_w2_words|selected_topfed_ffn_w2_words)\s*,[\s\S]*?(?:\(u32_t\)w2_weight_words|selected_topfed_ffn_w2_words_valid)[\s\S]*?(?:topfed_ffn_w2_bias_words|selected_topfed_ffn_w2_bias_words)\s*,[\s\S]*?(?:\(u32_t\)w2_bias_words|selected_topfed_ffn_w2_bias_words_valid)\s*,[\s\S]*?\(u32_t\)FFN_POLICY_REQUIRE_W2_TOPFED' -Reason "TransformerLayerTopManagedAttnBridge must dispatch strict top-fed W2 fallback policy flag"
Require-Regex -Text $layerText -Pattern '(?ms)FFNLayer0<\s*FFN_STAGE_W1\s*>\s*\([\s\S]*?(?:topfed_ffn_x_words|selected_topfed_ffn_x_words)\s*,[\s\S]*?(?:topfed_ffn_w1_words|selected_topfed_ffn_w1_words)\s*,[\s\S]*?(?:\(u32_t\)w1_weight_words|selected_topfed_ffn_w1_words_valid)[\s\S]*?\(u32_t\)FFN_POLICY_REQUIRE_W1_TOPFED[\s\S]*?(?:\(u32_t\)ffn_x_words|selected_topfed_ffn_x_words_valid)[\s\S]*?(?:topfed_ffn_w1_bias_words|selected_topfed_ffn_w1_bias_words)\s*,[\s\S]*?(?:\(u32_t\)w1_bias_words|selected_topfed_ffn_w1_bias_words_valid)' -Reason "TransformerLayer pointer path must dispatch strict top-fed W1 policy with x and W1-bias descriptors"
...
Require-Regex -Text $layerText -Pattern '(?ms)FFNLayer0<\s*FFN_STAGE_W2\s*>\s*\([\s\S]*?(?:topfed_ffn_w2_input_words|selected_topfed_ffn_w2_input_words)\s*,[\s\S]*?(?:\(u32_t\)w2_input_words|selected_topfed_ffn_w2_input_words_valid)[\s\S]*?(?:topfed_ffn_w2_words|selected_topfed_ffn_w2_words)\s*,[\s\S]*?(?:\(u32_t\)w2_weight_words|selected_topfed_ffn_w2_words_valid)[\s\S]*?(?:topfed_ffn_w2_bias_words|selected_topfed_ffn_w2_bias_words)\s*,[\s\S]*?(?:\(u32_t\)w2_bias_words|selected_topfed_ffn_w2_bias_words_valid)\s*,[\s\S]*?\(u32_t\)FFN_POLICY_REQUIRE_W2_TOPFED' -Reason "TransformerLayer pointer path must dispatch strict top-fed W2 fallback policy flag"
Require-Regex -Text $softmaxOutText -Pattern '(?ms)phase_tile_bridge_family_writeback_selected_consumed_count' -Reason "SoftmaxOut mainline missing W4-C4 writeback selected-consumed-count observability anchor"
Require-Regex -Text $topText -Pattern '(?ms)run_p11af_layer0_top_managed_softmax_out\s*\([\s\S]*?phase_tile_bridge_family_writeback_selected_consumed_count' -Reason "Top run_p11af helper missing W4-C4 writeback selected-consumed-count argument anchor"
...
Write-Log "guard: W4-B5 QkScore family bounded bridge anchors OK"
...
Write-Log "guard: W4-B3 QkScore selectable-head bounded score-tile bridge anchors OK"
Write-Log "guard: W4-B8 QkScore family bridge capacity expansion anchors OK"
Write-Log "guard: W4-B9 QkScore token-span family bridge anchors OK"
...
Write-Log "guard: W4-C0 SoftmaxOut family bridge contract anchors OK"
Write-Log "guard: W4-C1 SoftmaxOut head/token family descriptor contract anchors OK"
Write-Log "guard: W4-C2 SoftmaxOut ACC-path single later-token bridge anchors OK"
Write-Log "guard: W4-C3 SoftmaxOut RENORM-path single selected probe anchors OK"
Write-Log "guard: W4-C4 SoftmaxOut WRITEBACK-path single selected probe anchors OK"
