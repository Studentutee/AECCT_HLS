...
Require-Regex -Text $ffnText -Pattern '(?ms)if\s*\(\s*require_w1_topfed\s*&&\s*!\(w1_input_descriptor_ready\s*&&\s*w1_weight_descriptor_ready\)\s*\)\s*\{[\s\S]*?fallback_policy_reject_flag[\s\S]*?return\s*;' -Reason "FFNLayer0CoreWindow must reject W1 stage when strict top-fed descriptors are not ready"
...
Require-Regex -Text $layerText -Pattern '(?ms)FFNLayer0TopManagedWindowBridge<\s*FFN_STAGE_W1\s*>\s*\([\s\S]*?topfed_ffn_x_words\s*,[\s\S]*?topfed_ffn_w1_words\s*,[\s\S]*?\(u32_t\)w1_weight_words[\s\S]*?\(u32_t\)FFN_POLICY_REQUIRE_W1_TOPFED[\s\S]*?\(u32_t\)ffn_x_words' -Reason "TransformerLayerTopManagedAttnBridge must dispatch strict top-fed W1 fallback policy flag and x descriptor-ready valid"
...
Require-Regex -Text $layerText -Pattern '(?ms)FFNLayer0<\s*FFN_STAGE_W1\s*>\s*\([\s\S]*?topfed_ffn_x_words\s*,[\s\S]*?topfed_ffn_w1_words\s*,[\s\S]*?\(u32_t\)w1_weight_words[\s\S]*?\(u32_t\)FFN_POLICY_REQUIRE_W1_TOPFED[\s\S]*?\(u32_t\)ffn_x_words' -Reason "TransformerLayer pointer path must dispatch strict top-fed W1 fallback policy flag and x descriptor-ready valid"
