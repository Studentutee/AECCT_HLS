...
Require-Regex -Text $topText -Pattern '(?ms)param_ingest_one_word[\s\S]*?ingest_commit_diag_error\s*\([\s\S]*?RX_PARAM[\s\S]*?ERR_PARAM_LEN_MISMATCH' -Reason "PARAM commit-time diagnostics mapping missing"
Require-Regex -Text $topText -Pattern '(?ms)infer_ingest_one_word[\s\S]*?ingest_commit_diag_error\s*\([\s\S]*?RX_INFER' -Reason "INFER commit-time diagnostics helper path missing"
