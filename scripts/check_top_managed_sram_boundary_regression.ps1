...
Require-Regex -Text $ffnText -Pattern '(?ms)FFNLayer0CoreWindow\s*\([\s\S]*?u32_t\s+layer_id\s*=\s*\(u32_t\)0\s*,\s*const\s+u32_t\*\s+topfed_x_words\s*=\s*0\s*,\s*const\s+u32_t\*\s+topfed_w1_weight_words\s*=\s*0\s*,\s*u32_t\s+topfed_w1_weight_words_valid\s*=\s*0\s*,\s*const\s+u32_t\*\s+topfed_w2_input_words\s*=\s*0\s*,\s*u32_t\s+topfed_w2_input_words_valid\s*=\s*0\s*,\s*const\s+u32_t\*\s+topfed_w2_weight_words\s*=\s*0\s*,\s*u32_t\s+topfed_w2_weight_words_valid\s*=\s*0\s*,\s*const\s+u32_t\*\s+topfed_w2_bias_words\s*=\s*0\s*,\s*u32_t\s+topfed_w2_bias_words_valid\s*=\s*0\s*,\s*u32_t\s+fallback_policy_flags\s*=\s*\(u32_t\)FFN_POLICY_NONE' -Reason "FFNLayer0CoreWindow fallback policy arguments missing"
