#pragma once
// AecctProtocol.h
// SSOT for ctrl_cmd/ctrl_rsp packing and opcode/errcode enums.
//
...
//
// NOTE:
// - Unimplemented opcodes must return RSP_ERR(ERR_UNIMPL).
// - Opcode values must match spell v11.7.2.
...
namespace aecct {
  ...
  enum Opcode : unsigned {
    ...
    OP_SOFT_RESET  = 0x7F  // implemented (spell v11.7.2)
  };

  // -------------------- Top FSM state (M1) --------------------
  enum TopState : unsigned {
    ST_IDLE = 0,     // control command only
    ST_CFG_RX,       // cfg receive phase
    ST_PARAM_RX,     // param/weight receive phase
    ST_INFER_RX,     // infer input receive phase
