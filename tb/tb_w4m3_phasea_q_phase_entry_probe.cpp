// W4-M3: Phase-A Q phase-entry caller-fed x-row probe validation (local-only).
...
private:
    ...
    bool init_and_prepare() {
        ...
        if (!p11aeaf_tb::prepare_qkv_payload_set(payloads_)) {
            std::printf("[w4m3][FAIL] payload preparation failed\n");
            return false;
        }
        ...
        const bool q_mainline_taken = aecct::run_p11ad_layer0_top_managed_q(
            sram_expected_.data(),
            cfg_,
            ...
            pb_,
            fallback_taken);
        if (!q_mainline_taken || fallback_taken) {
            std::printf("[w4m3][FAIL] baseline P11AD run failed\n");
            return false;
        }
        return true;
    }
    ...
    bool run_positive_probe_visibility_case() {
        ...
        if (!q_mainline_taken || fallback_taken) {
            std::printf("[w4m3][FAIL] positive probe run did not stay on mainline\n");
            return false;
        }
        if ((uint32_t)probe_visible.to_uint() != 1u) {
            std::printf("[w4m3][FAIL] probe visibility flag mismatch\n");
            return false;
        }
        if ((uint32_t)probe_owner_ok.to_uint() != 1u) {
            std::printf("[w4m3][FAIL] probe ownership flag mismatch\n");
            return false;
        }
        if ((uint32_t)probe_compare_ok.to_uint() != 1u) {
            std::printf("[w4m3][FAIL] probe compare flag mismatch\n");
            return false;
        }
        std::printf("W4M3_PHASEAQ_CALLER_FED_XROW_VISIBLE PASS\n");
        std::printf("W4M3_PHASEAQ_OWNERSHIP_CHECK PASS\n");
        ...
        for (uint32_t i = 0u; i < tensor_words; ++i) {
            const uint32_t got_q = (uint32_t)sram_positive[q_base + i].to_uint();
            const uint32_t exp_q = (uint32_t)sram_expected_[q_base + i].to_uint();
            const uint32_t got_q_act = (uint32_t)sram_positive[q_act_q_base + i].to_uint();
            const uint32_t exp_q_act = (uint32_t)sram_expected_[q_act_q_base + i].to_uint();
            if (got_q != exp_q || got_q_act != exp_q_act) {
                std::printf("[w4m3][FAIL] expected compare mismatch idx=%u q=0x%08X exp_q=0x%08X q_act=0x%08X exp_q_act=0x%08X\n",
                    (unsigned)i, (unsigned)got_q, (unsigned)exp_q, (unsigned)got_q_act, (unsigned)exp_q_act);
                return false;
            }
        }
        if ((uint32_t)sram_positive[q_sx_base].to_uint() != (uint32_t)sram_expected_[q_sx_base].to_uint()) {
            std::printf("[w4m3][FAIL] expected compare mismatch q_sx got=0x%08X exp=0x%08X\n",
                (unsigned)(uint32_t)sram_positive[q_sx_base].to_uint(),
                (unsigned)(uint32_t)sram_expected_[q_sx_base].to_uint());
            return false;
        }
        std::printf("W4M3_PHASEAQ_EXPECTED_COMPARE PASS\n");
        ...
        for (uint32_t i = 0u; i < (uint32_t)sram_positive.size(); ++i) {
            ...
            if (before != after && allowed_write[i] == 0u) {
                std::printf("[w4m3][FAIL] spurious write addr=%u before=0x%08X after=0x%08X\n",
                    (unsigned)i, (unsigned)before, (unsigned)after);
                return false;
            }
        }
        std::printf("W4M3_PHASEAQ_NO_SPURIOUS_TOUCH PASS\n");
        return true;
    }
    ...
    bool run_negative_probe_reject_cases() {
...

        {
            ...
            const bool q_mainline_taken = aecct::run_p11ad_layer0_top_managed_q(
                ...
                &probe_compare_ok);
            if (q_mainline_taken || !fallback_taken) {
                std::printf("[w4m3][FAIL] owner-mismatch probe did not reject as expected\n");
                return false;
            }
            if ((uint32_t)probe_visible.to_uint() != 1u || (uint32_t)probe_owner_ok.to_uint() != 0u) {
                std::printf("[w4m3][FAIL] owner-mismatch observability flags mismatch\n");
                return false;
            }
            for (uint32_t i = 0u; i < (uint32_t)sram_neg_owner.size(); ++i) {
                if ((uint32_t)sram_neg_owner[i].to_uint() != (uint32_t)sram_init_[i].to_uint()) {
                    std::printf("[w4m3][FAIL] owner-mismatch reject wrote stale state addr=%u\n", (unsigned)i);
                    return false;
                }
            }
        }

        {
            ...
            const bool q_mainline_taken = aecct::run_p11ad_layer0_top_managed_q(
                ...
                &probe_compare_ok);
            if (q_mainline_taken || !fallback_taken) {
                std::printf("[w4m3][FAIL] compare-mismatch probe did not reject as expected\n");
                return false;
            }
            if ((uint32_t)probe_visible.to_uint() != 1u ||
                (uint32_t)probe_owner_ok.to_uint() != 1u ||
                (uint32_t)probe_compare_ok.to_uint() != 0u) {
                std::printf("[w4m3][FAIL] compare-mismatch observability flags mismatch\n");
                return false;
            }
            for (uint32_t i = 0u; i < (uint32_t)sram_neg_compare.size(); ++i) {
                if ((uint32_t)sram_neg_compare[i].to_uint() != (uint32_t)sram_init_[i].to_uint()) {
                    std::printf("[w4m3][FAIL] compare-mismatch reject wrote stale state addr=%u\n", (unsigned)i);
                    return false;
                }
            }
        }

        std::printf("W4M3_PHASEAQ_PROBE_MISMATCH_REJECT PASS\n");
        return true;
    }
};
...
CCS_MAIN(int argc, char** argv) {
    (void)argc;
    (void)argv;
    TbW4m3PhaseAQPhaseEntryProbe tb;
