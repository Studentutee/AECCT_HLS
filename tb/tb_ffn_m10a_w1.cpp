...
int main() {
    ...
    const uint32_t w1_words = (uint32_t)aecct::FFN_W1_OUT_WORDS;
    ...
    int rc = compare_tensor(
        "ffn_w1_out",
        sram,
        (uint32_t)sc.w1_out_base_word.to_uint(),
        trace_layer0_ffn_w1_out_fp_step0_tensor,
        sample_idx,
        w1_words,
        1.0e-5
    );
    ...
    if (rc != 0) {
        std::printf("ERROR: tb_ffn_m10a_w1 failed\n");
        return 1;
    }
