// W4-B2: QkScore bounded score-tile bridge validation (local-only).
...
public:
    int run_all() {
        ...
        std::printf("PASS: tb_w4b2_qkscore_tile_bridge\n");
        return 0;
    }
...
private:
    ...
    bool init_and_bootstrap() {
        ...
        if (!p11aeaf_tb::prepare_qkv_payload_set(payloads_)) {
            std::printf("[w4b2][FAIL] payload preparation failed\n");
            return false;
        }
        ...
        if (!p11aeaf_tb::run_ac_ad_mainline(sram_, q_fallback_taken, kv_fallback_taken)) {
            std::printf("[w4b2][FAIL] AC/AD bootstrap failed\n");
            return false;
        }
        if (q_fallback_taken || kv_fallback_taken) {
            std::printf("[w4b2][FAIL] AC/AD bootstrap fallback detected\n");
            return false;
        }

        bridge_key_begin_ = 0u;
        ...
        bridge_words_valid_ = token_count < tile_words ? token_count : tile_words;
        if (bridge_words_valid_ == 0u) {
            std::printf("[w4b2][FAIL] invalid bridge_words_valid\n");
            return false;
        }
        return true;
    }
    ...
    bool run_positive_tile_bridge_case() {
        ...
        if (!score_mainline_taken || fallback_taken) {
            std::printf("[w4b2][FAIL] positive bridge run did not stay on mainline\n");
            return false;
        }
        if ((uint32_t)bridge_visible.to_uint() != 1u ||
            ...
            std::printf("[w4b2][FAIL] bridge observability flags mismatch\n");
            return false;
        }
        std::printf("W4B2_QKSCORE_TILE_BRIDGE_VISIBLE PASS\n");
        std::printf("W4B2_QKSCORE_OWNERSHIP_CHECK PASS\n");
        ...
        for (uint32_t h = 0u; h < n_heads; ++h) {
            ...
            for (uint32_t j = 0u; j < token_count; ++j) {
                ...
                if (got != exp) {
                    std::printf("[w4b2][FAIL] expected compare mismatch head=%u key=%u got=0x%08X exp=0x%08X\n",
                        (unsigned)h, (unsigned)j, (unsigned)got, (unsigned)exp);
                    return false;
                }
            }
        }
        std::printf("W4B2_QKSCORE_EXPECTED_COMPARE PASS\n");
        ...
        for (uint32_t i = 0u; i < (uint32_t)sram_.size(); ++i) {
            ...
            if (before != after && allowed_write[i] == 0u) {
                std::printf("[w4b2][FAIL] spurious write addr=%u before=0x%08X after=0x%08X\n",
                    (unsigned)i, (unsigned)before, (unsigned)after);
                return false;
            }
        }
        std::printf("W4B2_QKSCORE_NO_SPURIOUS_TOUCH PASS\n");
        return true;
    }
    ...
    bool run_negative_tile_bridge_mismatch_case() {
        ...
        if (score_mainline_taken || !fallback_taken) {
            std::printf("[w4b2][FAIL] mismatch bridge did not reject as expected\n");
            return false;
        }
        if ((uint32_t)bridge_visible.to_uint() != 1u ||
            (uint32_t)bridge_owner_ok.to_uint() != 1u ||
            ...
            std::printf("[w4b2][FAIL] mismatch observability flags mismatch\n");
            return false;
        }
        for (uint32_t i = 0u; i < (uint32_t)sram_negative.size(); ++i) {
            if ((uint32_t)sram_negative[i].to_uint() != (uint32_t)sram_before_positive_[i].to_uint()) {
                std::printf("[w4b2][FAIL] reject path wrote stale state addr=%u\n", (unsigned)i);
                return false;
            }
        }
        std::printf("W4B2_QKSCORE_BRIDGE_MISMATCH_REJECT PASS\n");
        return true;
    }
};
