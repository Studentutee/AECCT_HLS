...
private:
    ...
    static const char* attn_shell_stage_name(aecct::TransformerAttnCompatShellStage stage) {
        ...
        if (stage == aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY) {
            return "QKV_SCORES_ONLY";
        }
        return "OUT_ONLY";
    }
    ...
    static aecct::TransformerAttnCompatShellStage attn_shell_stage_reference_model(
        ...
        const bool kv_ready_q_not_prebuilt_score_ready_partial_out_stage_payload_enabled_bucket =
            kv_prebuilt_from_top_managed &&
            !q_prebuilt_from_top_managed &&
            score_prebuilt_from_top_managed &&
            !out_prebuilt_from_top_managed &&
            attn_out_topfed_payload_enable;
        if (kv_ready_q_not_prebuilt_score_ready_partial_out_stage_payload_enabled_bucket) {
            return aecct::TRANSFORMER_ATTN_COMPAT_SHELL_OUT_ONLY;
        }
        const bool qkv_not_prebuilt_score_ready_partial_out_stage_bucket =
            !kv_prebuilt_from_top_managed &&
            !q_prebuilt_from_top_managed &&
            score_prebuilt_from_top_managed &&
            !out_prebuilt_from_top_managed &&
        ...
        const bool selected_partial_qkv_scores_stage_bucket =
            ...
            !out_prebuilt_from_top_managed &&
            !attn_out_topfed_payload_enable;
        if (selected_partial_qkv_scores_stage_bucket) {
            return aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY;
        }
        const bool selected_partial_qkv_scores_stage_payload_enabled_bucket =
            !kv_prebuilt_from_top_managed &&
            q_prebuilt_from_top_managed &&
            !score_prebuilt_from_top_managed &&
            !out_prebuilt_from_top_managed &&
            attn_out_topfed_payload_enable;
        if (selected_partial_qkv_scores_stage_payload_enabled_bucket) {
            return aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY;
        }
        const bool kv_ready_q_not_prebuilt_qkv_scores_stage_bucket =
            kv_prebuilt_from_top_managed &&
            !q_prebuilt_from_top_managed &&
            !score_prebuilt_from_top_managed &&
            !out_prebuilt_from_top_managed &&
            !attn_out_topfed_payload_enable;
        if (kv_ready_q_not_prebuilt_qkv_scores_stage_bucket) {
            return aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY;
        }
        const bool kv_ready_q_not_prebuilt_qkv_scores_stage_payload_enabled_bucket =
            kv_prebuilt_from_top_managed &&
            !q_prebuilt_from_top_managed &&
            !score_prebuilt_from_top_managed &&
            !out_prebuilt_from_top_managed &&
            attn_out_topfed_payload_enable;
        if (kv_ready_q_not_prebuilt_qkv_scores_stage_payload_enabled_bucket) {
            return aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY;
        }
        const bool qkv_not_prebuilt_qkv_scores_stage_bucket =
            !kv_prebuilt_from_top_managed &&
            !q_prebuilt_from_top_managed &&
            !score_prebuilt_from_top_managed &&
            !out_prebuilt_from_top_managed &&
            !attn_out_topfed_payload_enable;
        if (qkv_not_prebuilt_qkv_scores_stage_bucket) {
            return aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY;
        }
        const bool qkv_not_prebuilt_qkv_scores_stage_payload_enabled_bucket =
            !kv_prebuilt_from_top_managed &&
            !q_prebuilt_from_top_managed &&
            !score_prebuilt_from_top_managed &&
            !out_prebuilt_from_top_managed &&
            attn_out_topfed_payload_enable;
        if (qkv_not_prebuilt_qkv_scores_stage_payload_enabled_bucket) {
            return aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY;
        }
    ...
    bool run_attn_compat_shell_selector_truth_table_audit() const {
        ...
        bool q_ready_kv_not_prebuilt_payload_enabled_to_qkv_scores_stage_ok = false;
        bool kv_ready_q_not_prebuilt_to_qkv_scores_stage_ok = false;
        bool kv_ready_q_not_prebuilt_payload_enabled_to_qkv_scores_stage_ok = false;
        bool qkv_not_prebuilt_to_qkv_scores_stage_ok = false;
        bool qkv_not_prebuilt_payload_enabled_to_qkv_scores_stage_ok = false;
        ...
        for (uint32_t bits = 0u; bits < 16u; ++bits) {
            ...
            for (uint32_t payload = 0u; payload < 2u; ++payload) {
                ...
                const bool q_ready_kv_not_prebuilt_bucket =
                    ...
                    !out_prebuilt &&
                    !payload_enable;
                const bool q_ready_kv_not_prebuilt_payload_enabled_bucket =
                    !kv_prebuilt &&
                    q_prebuilt &&
                    !score_prebuilt &&
                    !out_prebuilt &&
                    payload_enable;
                const bool kv_ready_q_not_prebuilt_bucket =
                    kv_prebuilt &&
                    !q_prebuilt &&
                    !score_prebuilt &&
                    !out_prebuilt &&
                    !payload_enable;
                const bool kv_ready_q_not_prebuilt_payload_enabled_bucket =
                    kv_prebuilt &&
                    !q_prebuilt &&
                    !score_prebuilt &&
                    !out_prebuilt &&
                    payload_enable;
                const bool qkv_not_prebuilt_bucket =
                    !kv_prebuilt &&
                    !q_prebuilt &&
                    !score_prebuilt &&
                    !out_prebuilt &&
                    !payload_enable;
                const bool qkv_not_prebuilt_payload_enabled_bucket =
                    !kv_prebuilt &&
                    !q_prebuilt &&
                    !score_prebuilt &&
                    !out_prebuilt &&
                ...
                const bool kv_ready_q_not_prebuilt_score_ready_bucket =
                    ...
                    !out_prebuilt &&
                    !payload_enable;
                const bool kv_ready_q_not_prebuilt_score_ready_payload_enabled_bucket =
                    kv_prebuilt &&
                    !q_prebuilt &&
                    score_prebuilt &&
                    !out_prebuilt &&
                    payload_enable;
                const bool qkv_not_prebuilt_score_ready_bucket =
                    !kv_prebuilt &&
                    !q_prebuilt &&
                    score_prebuilt &&
                    !out_prebuilt &&
                ...
                if (q_ready_kv_not_prebuilt_bucket &&
                    got == aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY) {
                    q_ready_kv_not_prebuilt_to_qkv_scores_stage_ok = true;
                }
                if (q_ready_kv_not_prebuilt_payload_enabled_bucket &&
                    got == aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY) {
                    q_ready_kv_not_prebuilt_payload_enabled_to_qkv_scores_stage_ok = true;
                }
                if (kv_ready_q_not_prebuilt_bucket &&
                    got == aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY) {
                    kv_ready_q_not_prebuilt_to_qkv_scores_stage_ok = true;
                }
                if (kv_ready_q_not_prebuilt_payload_enabled_bucket &&
                    got == aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY) {
                    kv_ready_q_not_prebuilt_payload_enabled_to_qkv_scores_stage_ok = true;
                }
                if (qkv_not_prebuilt_bucket &&
                    got == aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY) {
                    qkv_not_prebuilt_to_qkv_scores_stage_ok = true;
                }
                if (qkv_not_prebuilt_payload_enabled_bucket &&
                    got == aecct::TRANSFORMER_ATTN_COMPAT_SHELL_QKV_SCORES_ONLY) {
                    qkv_not_prebuilt_payload_enabled_to_qkv_scores_stage_ok = true;
                }
                if (q_ready_kv_not_prebuilt_score_ready_bucket &&
                    got == aecct::TRANSFORMER_ATTN_COMPAT_SHELL_OUT_ONLY) {
                ...
                if (kv_ready_q_not_prebuilt_score_ready_payload_enabled_bucket &&
                    got == aecct::TRANSFORMER_ATTN_COMPAT_SHELL_OUT_ONLY) {
                    kv_ready_q_not_prebuilt_score_ready_payload_enabled_to_out_stage_ok = true;
                }
                ...
                if (!fully_prebuilt && !selected_partial_bucket &&
                    ...
                    !q_ready_kv_not_prebuilt_payload_enabled_bucket &&
                    !kv_ready_q_not_prebuilt_bucket &&
                    !kv_ready_q_not_prebuilt_payload_enabled_bucket &&
                    !qkv_not_prebuilt_bucket &&
                    !qkv_not_prebuilt_payload_enabled_bucket &&
        ...
        if (!q_ready_kv_not_prebuilt_to_qkv_scores_stage_ok) {
            std::printf("[p11aj][FAIL] q-ready kv-not-prebuilt bucket did not map to QKV_SCORES_ONLY\n");
            return false;
        }
        if (!q_ready_kv_not_prebuilt_payload_enabled_to_qkv_scores_stage_ok) {
            std::printf("[p11aj][FAIL] q-ready kv-not-prebuilt payload-enabled bucket did not map to QKV_SCORES_ONLY\n");
            return false;
        }
        if (!kv_ready_q_not_prebuilt_to_qkv_scores_stage_ok) {
            std::printf("[p11aj][FAIL] kv-ready q-not-prebuilt bucket did not map to QKV_SCORES_ONLY\n");
            return false;
        }
        if (!kv_ready_q_not_prebuilt_payload_enabled_to_qkv_scores_stage_ok) {
            std::printf("[p11aj][FAIL] kv-ready q-not-prebuilt payload-enabled bucket did not map to QKV_SCORES_ONLY\n");
            return false;
        }
        if (!qkv_not_prebuilt_to_qkv_scores_stage_ok) {
            std::printf("[p11aj][FAIL] q-not-prebuilt kv-not-prebuilt bucket did not map to QKV_SCORES_ONLY\n");
            return false;
        }
        if (!qkv_not_prebuilt_payload_enabled_to_qkv_scores_stage_ok) {
            std::printf("[p11aj][FAIL] q-not-prebuilt kv-not-prebuilt payload-enabled bucket did not map to QKV_SCORES_ONLY\n");
            return false;
        }
        if (!q_ready_kv_not_prebuilt_score_ready_to_out_stage_ok) {
        ...
        if (!kv_ready_q_not_prebuilt_score_ready_payload_enabled_to_out_stage_ok) {
            std::printf("[p11aj][FAIL] kv-ready q-not-prebuilt score-ready payload-enabled bucket did not map to OUT_ONLY\n");
            return false;
        }
        ...
        std::printf("Q_READY_KV_NOT_PREBUILT_PAYLOAD_ENABLED_TO_QKV_SCORES_STAGE PASS\n");
        std::printf("Q_READY_KV_NOT_PREBUILT_PAYLOAD_ENABLED_QKV_SCORES_STAGE_MIGRATION PASS\n");
        std::printf("KV_READY_Q_NOT_PREBUILT_TO_QKV_SCORES_STAGE PASS\n");
        std::printf("KV_READY_Q_NOT_PREBUILT_QKV_SCORES_STAGE_MIGRATION PASS\n");
        std::printf("KV_READY_Q_NOT_PREBUILT_PAYLOAD_ENABLED_TO_QKV_SCORES_STAGE PASS\n");
        std::printf("KV_READY_Q_NOT_PREBUILT_PAYLOAD_ENABLED_QKV_SCORES_STAGE_MIGRATION PASS\n");
        std::printf("QKV_NOT_PREBUILT_TO_QKV_SCORES_STAGE PASS\n");
        std::printf("QKV_NOT_PREBUILT_QKV_SCORES_STAGE_MIGRATION PASS\n");
        std::printf("QKV_NOT_PREBUILT_PAYLOAD_ENABLED_TO_QKV_SCORES_STAGE PASS\n");
        std::printf("QKV_NOT_PREBUILT_PAYLOAD_ENABLED_QKV_SCORES_STAGE_MIGRATION PASS\n");
