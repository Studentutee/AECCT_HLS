// W4-B8: QkScore bounded family full-head mixed coverage validation (local-only).
...
class TbW4b8QkScoreFamilyFullheadBridge {
public:
    int run_all() {
        if (!init_and_bootstrap()) { return 1; }
        if (!run_positive_fullhead_mixed_case()) { return 1; }
        if (!run_negative_fullhead_mismatch_case()) { return 1; }
        std::printf("PASS: tb_w4b8_qkscore_family_fullhead_bridge\n");
        return 0;
    }
...
private:
    ...
    bool init_and_bootstrap() {
        ...
        if (!p11aeaf_tb::prepare_qkv_payload_set(payloads_)) {
            std::printf("[w4b8][FAIL] payload preparation failed\n");
            return false;
        }
        ...
        if (!p11aeaf_tb::run_ac_ad_mainline(sram_bootstrap_, q_fallback_taken, kv_fallback_taken)) {
            std::printf("[w4b8][FAIL] AC/AD bootstrap failed\n");
            return false;
        }
        if (q_fallback_taken || kv_fallback_taken) {
            std::printf("[w4b8][FAIL] AC/AD bootstrap fallback detected\n");
            return false;
        }
        ...
        if (token_count < 2u || n_heads < 2u || tile_words == 0u) {
            std::printf("[w4b8][FAIL] invalid topology for full-head mixed bridge\n");
            return false;
        }

        // W4-B8 target: one-call full-head mixed coverage.
        case_count_ = (n_heads > 1u) ? (n_heads - 1u) : 0u;
        if (case_count_ == 0u || case_count_ >= kMaxFamilyCases) {
            std::printf("[w4b8][FAIL] invalid full-head mixed case_count=%u max=%u\n",
                (unsigned)case_count_, (unsigned)kMaxFamilyCases);
            return false;
        }
        for (uint32_t c = 0u; c < case_count_; ++c) {
            case_head_[c] = c;
            uint32_t begin = (c * 5u) % token_count;
            uint32_t valid = ((tile_words >= 2u) && ((c & 1u) == 0u)) ? 2u : 1u;
            if (valid > token_count) {
                valid = token_count;
            }
            if ((begin + valid) > token_count) {
                begin = token_count - valid;
            }
            case_key_begin_[c] = begin;
            case_valid_words_[c] = valid;
        }

        case_total_words_ = 0u;
        for (uint32_t c = 0u; c < case_count_; ++c) {
            if (case_valid_words_[c] == 0u || case_valid_words_[c] > tile_words) {
                std::printf("[w4b8][FAIL] invalid case_valid_words for case %u\n", (unsigned)c);
                return false;
            }
            if (case_key_begin_[c] >= token_count ||
                (case_key_begin_[c] + case_valid_words_[c]) > token_count) {
                std::printf("[w4b8][FAIL] invalid case key range for case %u\n", (unsigned)c);
                return false;
            }
            case_total_words_ += case_valid_words_[c];
        }
        if (n_heads <= case_count_) {
            std::printf("[w4b8][FAIL] full-head mixed bridge requires one single-only head\n");
            return false;
        }
        single_head_ = case_count_;
        single_key_begin_ = (token_count > 4u) ? 3u : 0u;
    ...
    bool run_positive_fullhead_mixed_case() {
        ...
        if (!score_mainline_taken_bridge || fallback_taken_bridge) {
            std::printf("[w4b8][FAIL] mixed bridge run did not stay on mainline\n");
            return false;
        }
        ...
        if ((uint32_t)family_visible_count.to_uint() != case_count_ ||
            ...
            std::printf("[w4b8][FAIL] family observability mismatch visible=%u owner=%u consumed=%u compare=%u mask=0x%X expected_mask=0x%X\n",
        ...
        if ((uint32_t)single_visible.to_uint() != 1u ||
            ...
            std::printf("[w4b8][FAIL] single observability mismatch visible=%u owner=%u consumed=%u compare=%u\n",
        ...
        std::printf("W4B8_QKSCORE_FAMILY_FULLHEAD_BRIDGE_VISIBLE PASS\n");
        std::printf("W4B8_QKSCORE_FAMILY_FULLHEAD_OWNERSHIP_CHECK PASS\n");
        std::printf("W4B8_QKSCORE_FAMILY_FULLHEAD_MULTI_PATH_ANTI_FALLBACK PASS\n");
        ...
        if (!score_mainline_taken_legacy || fallback_taken_legacy) {
            std::printf("[w4b8][FAIL] legacy baseline run did not stay on mainline\n");
            return false;
        }
        ...
        for (uint32_t h = 0u; h < n_heads; ++h) {
            ...
            for (uint32_t j = 0u; j < token_count; ++j) {
                ...
                if (got_bridge != exp) {
                    std::printf("[w4b8][FAIL] expected compare mismatch head=%u key=%u got=0x%08X exp=0x%08X\n",
                        (unsigned)h, (unsigned)j, (unsigned)got_bridge, (unsigned)exp);
                    return false;
                }
                if (got_bridge != got_legacy) {
                    std::printf("[w4b8][FAIL] legacy compare mismatch head=%u key=%u bridge=0x%08X legacy=0x%08X\n",
                        (unsigned)h, (unsigned)j, (unsigned)got_bridge, (unsigned)got_legacy);
                    return false;
                }
            }
        }
        std::printf("W4B8_QKSCORE_FAMILY_FULLHEAD_EXPECTED_COMPARE PASS\n");
        std::printf("W4B8_QKSCORE_FAMILY_FULLHEAD_LEGACY_COMPARE PASS\n");
        ...
        for (uint32_t i = 0u; i < (uint32_t)sram_bridge.size(); ++i) {
            ...
            if (before != after && allowed_write[i] == 0u) {
                std::printf("[w4b8][FAIL] spurious write addr=%u before=0x%08X after=0x%08X\n",
                    (unsigned)i, (unsigned)before, (unsigned)after);
                return false;
            }
        }
        std::printf("W4B8_QKSCORE_FAMILY_FULLHEAD_NO_SPURIOUS_TOUCH PASS\n");
        return true;
    }

    bool run_negative_fullhead_mismatch_case() {
        ...
        if (score_mainline_taken || !fallback_taken) {
            std::printf("[w4b8][FAIL] mismatch mixed bridge did not reject as expected\n");
            return false;
        }
        if ((uint32_t)family_visible_count.to_uint() != 1u ||
            ...
            std::printf("[w4b8][FAIL] mismatch observability flags mismatch visible=%u owner=%u consumed=%u compare=%u mask=0x%X\n",
                (unsigned)(uint32_t)family_visible_count.to_uint(),
                (unsigned)(uint32_t)family_owner_ok.to_uint(),
                (unsigned)(uint32_t)family_consumed_count.to_uint(),
                (unsigned)(uint32_t)family_compare_ok.to_uint(),
        ...
        if ((uint32_t)single_visible.to_uint() != 0u ||
            ...
            std::printf("[w4b8][FAIL] mismatch single flags expected zero visible=%u owner=%u consumed=%u compare=%u\n",
                (unsigned)(uint32_t)single_visible.to_uint(),
                (unsigned)(uint32_t)single_owner_ok.to_uint(),
                (unsigned)(uint32_t)single_consumed.to_uint(),
                (unsigned)(uint32_t)single_compare_ok.to_uint());
        ...
        for (uint32_t i = 0u; i < (uint32_t)sram_negative.size(); ++i) {
            if ((uint32_t)sram_negative[i].to_uint() !=
                ...
                std::printf("[w4b8][FAIL] reject path wrote stale state addr=%u\n", (unsigned)i);
                return false;
            }
        }
        std::printf("W4B8_QKSCORE_FAMILY_FULLHEAD_MISMATCH_REJECT PASS\n");
        return true;
    }
};
...
CCS_MAIN(int argc, char** argv) {
    (void)argc;
    (void)argv;
    TbW4b8QkScoreFamilyFullheadBridge tb;
