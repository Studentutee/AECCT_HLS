// W4-B5: QkScore bounded bridge family generalization validation (local-only).
...
class TbW4b5QkScoreFamilyBridge {
public:
    int run_all() {
        if (!init_and_bootstrap()) { return 1; }
        if (!run_positive_family_case()) { return 1; }
        if (!run_negative_family_mismatch_case()) { return 1; }
        std::printf("PASS: tb_w4b5_qkscore_family_bridge\n");
        return 0;
    }

private:
    static const uint32_t kMaxFamilyCases = 3u;
    ...
    bool init_and_bootstrap() {
        ...
        if (!p11aeaf_tb::prepare_qkv_payload_set(payloads_)) {
            std::printf("[w4b5][FAIL] payload preparation failed\n");
            return false;
        }
        ...
        if (!p11aeaf_tb::run_ac_ad_mainline(sram_bootstrap_, q_fallback_taken, kv_fallback_taken)) {
            std::printf("[w4b5][FAIL] AC/AD bootstrap failed\n");
            return false;
        }
        if (q_fallback_taken || kv_fallback_taken) {
            std::printf("[w4b5][FAIL] AC/AD bootstrap fallback detected\n");
            return false;
        }
        ...
        if (token_count < 2u || n_heads < 2u || tile_words == 0u) {
            std::printf("[w4b5][FAIL] invalid topology for family bridge\n");
            return false;
        }

        case_count_ = n_heads >= 3u ? 3u : 2u;
        case_head_[0] = 0u;
        ...
        for (uint32_t c = 0u; c < case_count_; ++c) {
            if (case_valid_words_[c] == 0u || case_valid_words_[c] > tile_words) {
                std::printf("[w4b5][FAIL] invalid case_valid_words for case %u\n", (unsigned)c);
                return false;
            }
            if (case_key_begin_[c] >= token_count ||
                (case_key_begin_[c] + case_valid_words_[c]) > token_count) {
                std::printf("[w4b5][FAIL] invalid case key range for case %u\n", (unsigned)c);
                return false;
            }
    ...
    bool run_positive_family_case() {
        ...
        if (!score_mainline_taken_bridge || fallback_taken_bridge) {
            std::printf("[w4b5][FAIL] family bridge run did not stay on mainline\n");
            return false;
        }
        ...
        if ((uint32_t)family_visible_count.to_uint() != case_count_ ||
            ...
            std::printf("[w4b5][FAIL] family observability mismatch visible=%u owner=%u consumed=%u compare=%u mask=0x%X expected_mask=0x%X\n",
        ...
        std::printf("W4B5_QKSCORE_FAMILY_BRIDGE_VISIBLE PASS\n");
        std::printf("W4B5_QKSCORE_FAMILY_OWNERSHIP_CHECK PASS\n");
        std::printf("W4B5_QKSCORE_FAMILY_MULTI_CASE_ANTI_FALLBACK PASS\n");
        ...
        if (!score_mainline_taken_legacy || fallback_taken_legacy) {
            std::printf("[w4b5][FAIL] legacy baseline run did not stay on mainline\n");
            return false;
        }
        ...
        for (uint32_t h = 0u; h < n_heads; ++h) {
            ...
            for (uint32_t j = 0u; j < token_count; ++j) {
                ...
                if (got_bridge != exp) {
                    std::printf("[w4b5][FAIL] expected compare mismatch head=%u key=%u got=0x%08X exp=0x%08X\n",
                        (unsigned)h, (unsigned)j, (unsigned)got_bridge, (unsigned)exp);
                    return false;
                }
                if (got_bridge != got_legacy) {
                    std::printf("[w4b5][FAIL] legacy compare mismatch head=%u key=%u bridge=0x%08X legacy=0x%08X\n",
                        (unsigned)h, (unsigned)j, (unsigned)got_bridge, (unsigned)got_legacy);
                    return false;
                }
            }
        }
        std::printf("W4B5_QKSCORE_FAMILY_EXPECTED_COMPARE PASS\n");
        std::printf("W4B5_QKSCORE_FAMILY_LEGACY_COMPARE PASS\n");
        ...
        for (uint32_t i = 0u; i < (uint32_t)sram_bridge.size(); ++i) {
            ...
            if (before != after && allowed_write[i] == 0u) {
                std::printf("[w4b5][FAIL] spurious write addr=%u before=0x%08X after=0x%08X\n",
                    (unsigned)i, (unsigned)before, (unsigned)after);
                return false;
            }
        }
        std::printf("W4B5_QKSCORE_FAMILY_NO_SPURIOUS_TOUCH PASS\n");
        return true;
    }
    ...
    bool run_negative_family_mismatch_case() {
        ...
        if (score_mainline_taken || !fallback_taken) {
            std::printf("[w4b5][FAIL] mismatch family bridge did not reject as expected\n");
            return false;
        }
        if ((uint32_t)family_visible_count.to_uint() != 1u ||
            ...
            std::printf("[w4b5][FAIL] mismatch observability flags mismatch visible=%u owner=%u consumed=%u compare=%u mask=0x%X\n",
                (unsigned)(uint32_t)family_visible_count.to_uint(),
                (unsigned)(uint32_t)family_owner_ok.to_uint(),
                (unsigned)(uint32_t)family_consumed_count.to_uint(),
                (unsigned)(uint32_t)family_compare_ok.to_uint(),
        ...
        for (uint32_t i = 0u; i < (uint32_t)sram_negative.size(); ++i) {
            if ((uint32_t)sram_negative[i].to_uint() !=
                ...
                std::printf("[w4b5][FAIL] reject path wrote stale state addr=%u\n", (unsigned)i);
                return false;
            }
        }
        std::printf("W4B5_QKSCORE_FAMILY_MISMATCH_REJECT PASS\n");
        return true;
    }
};
