...
int main() {
    ...
    for (uint32_t i = 0; i < words; ++i) {
        sram[(uint32_t)sc.post_concat_base_word.to_uint() + i] =
            (aecct::u32_t)f32_to_bits((float)trace_layer0_attn_post_concat_step0_tensor[sample_idx * words + i]);
    }
    ...
    int rc = compare_tensor(
        ...
        trace_layer0_attn_out_step0_tensor,
        sample_idx,
        words,
        1.0e-5
    );
    ...
    if (rc != 0) {
        std::printf("ERROR: tb_attn_m9c_out failed\n");
        return 1;
    }

    std::printf("PASS: tb_attn_m9c_out\n");
    return 0;
}
