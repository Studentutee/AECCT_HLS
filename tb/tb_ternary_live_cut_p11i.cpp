// tb_ternary_live_cut_p11i.cpp
// P00-011I: third source-side call-site cut for live ternary L0_WQ + L0_WK + L0_WV.
...
static void validate_record_or_die(
    ...
    if (rec.weight_param_id != meta.weight_param_id ||
        ...
        std::printf("[p11i][FAIL] %s JSON record does not match current metadata\n", matrix_name);
        std::exit(1);
    }
    if (rec.payload_words.size() != rec.payload_words_2b) {
        std::printf("[p11i][FAIL] %s payload_words size mismatch\n", matrix_name);
        std::exit(1);
    }
    ...
    if (rec.inv_sw_bits != expect_inv_sw_bits) {
        std::printf("[p11i][FAIL] %s inv_s_w JSON record does not match weights.h reference\n", matrix_name);
        std::exit(1);
    }
}
...
static void expect_rsp(aecct::ctrl_ch_t& ctrl_rsp, uint8_t expect_kind, uint8_t expect_payload) {
    ...
    if (kind != expect_kind || payload != expect_payload) {
        std::printf("[p11i][FAIL] rsp mismatch kind=%u payload=%u expect_kind=%u expect_payload=%u\n",
                    (unsigned)kind,
                    (unsigned)payload,
...
static void expect_state(aecct::TopState expect_s) {
    ...
    if ((unsigned)got != (unsigned)expect_s) {
        std::printf("[p11i][FAIL] state mismatch got=%u expect=%u\n",
                    (unsigned)got,
                    (unsigned)expect_s);
        std::exit(1);
    }
...
static void compare_exact_or_die(const char* label, uint32_t got_bits, uint32_t expect_bits, uint32_t idx) {
    if (got_bits != expect_bits) {
        std::printf("[p11i][FAIL] %s mismatch idx=%u got=0x%08X expect=0x%08X got_f=%g expect_f=%g\n",
                    label,
                    (unsigned)idx,
                    (unsigned)got_bits,
...
int main() {
    ...
    if (!parse_matrix_export_record(json_text, "L0_WV", wv_rec)) {
        fail("failed to parse L0_WV record from gen/ternary_p11c_export.json");
    }
    ...
    validate_record_or_die("L0_WV", wv_rec, wv_meta, w_decoder_layers_0_self_attn_linears_2_s_w[0]);
    if (wk_rec.rows != wq_rec.rows || wk_rec.cols != wq_rec.cols ||
        wv_rec.rows != wq_rec.rows || wv_rec.cols != wq_rec.cols) {
        fail("L0_WQ/L0_WK/L0_WV dimensions must match in tb_ternary_live_cut_p11i");
    }
    ...
    const ParamMeta wv_payload_meta = kParamMeta[wv_rec.weight_param_id];
    const ParamMeta wv_inv_meta = kParamMeta[wv_rec.inv_sw_param_id];
    ...
    for (uint32_t i = 0u; i < wv_rec.payload_words_2b; ++i) {
        param_words[wv_payload_meta.offset_w + i] = wv_rec.payload_words[i];
    }
    param_words[wq_inv_meta.offset_w] = wq_rec.inv_sw_bits;
    param_words[wk_inv_meta.offset_w] = wk_rec.inv_sw_bits;
    param_words[wv_inv_meta.offset_w] = wv_rec.inv_sw_bits;
    ...
    std::printf("[p11i][PASS] fallback path retained: Q/K/V copy + Q/K/V_act_q copy + Q_sx=1\n");
    ...
    const uint32_t wv_payload_base = param_base_word + wv_payload_meta.offset_w;
    const uint32_t wv_inv_base = param_base_word + wv_inv_meta.offset_w;
    ...
    compare_exact_or_die("WV payload first word", (uint32_t)sram[wv_payload_base + 0u].to_uint(), wv_rec.payload_words[0u], 0u);
    compare_exact_or_die("WV payload last word", (uint32_t)sram[wv_payload_base + wv_rec.payload_words_2b - 1u].to_uint(), wv_rec.payload_words[wv_rec.payload_words_2b - 1u], wv_rec.payload_words_2b - 1u);
    compare_exact_or_die("WV inv_s_w word", (uint32_t)sram[wv_inv_base].to_uint(), wv_rec.inv_sw_bits, 0u);
    ...
    std::printf("[p11i][PASS] direct WK inv_s_w helper read exact match bits=0x%08X\n",
                (unsigned)wk_rec.inv_sw_bits);

    // direct positive WV metadata binding proof
    aecct::u32_t wv_helper_inv_sw_bits = (aecct::u32_t)0u;
    if (!aecct::ternary_linear_live_l0_wv_read_inv_sw_bits(
            sram,
            (aecct::u32_t)param_base_word,
            wv_helper_inv_sw_bits)) {
        fail("ternary_linear_live_l0_wv_read_inv_sw_bits returned false");
    }
    compare_exact_or_die(
        "WV helper inv_s_w read",
        (uint32_t)wv_helper_inv_sw_bits.to_uint(),
        wv_rec.inv_sw_bits,
        0u
    );
    std::printf("[p11i][PASS] direct WV inv_s_w helper read exact match bits=0x%08X\n",
                (unsigned)wv_rec.inv_sw_bits);
    ...
    const aecct::quant_acc_t wv_inv_sw = inv_sw_from_bits(wv_rec.inv_sw_bits);
    uint32_t live_q_diff_count = 0u;
    uint32_t live_k_diff_count = 0u;
    uint32_t live_v_diff_count = 0u;
    ...
    for (uint32_t out = 0u; out < wv_rec.rows; ++out) {
        const uint32_t expect_v_bits = compute_expect_q_elem_bits(
            sram, x_in_base, wv_rec.cols, w_decoder_layers_0_self_attn_linears_2_weight, out, wv_inv_sw);
        const uint32_t got_v_bits = (uint32_t)sram[(uint32_t)sc.v_base_word.to_uint() + out].to_uint();
        const uint32_t got_v_act_q_bits = (uint32_t)sram[(uint32_t)sc.v_act_q_base_word.to_uint() + out].to_uint();
        const uint32_t x_bits = (uint32_t)sram[x_in_base + out].to_uint();
        compare_exact_or_die("live V", got_v_bits, expect_v_bits, out);
        compare_exact_or_die("live V_act_q", got_v_act_q_bits, expect_v_bits, out);
        if (got_v_bits != x_bits) {
            ++live_v_diff_count;
        }
    }
    if (live_v_diff_count == 0u) {
        fail("live helper path did not alter any V element");
    }
    compare_exact_or_die("live Q_sx", (uint32_t)sram[(uint32_t)sc.q_sx_base_word.to_uint()].to_uint(), wq_rec.inv_sw_bits, 0u);

    std::printf("[p11i][PASS] source-side live call-site exercised for L0_WQ diff_count=%u\n",
                (unsigned)live_q_diff_count);
    std::printf("[p11i][PASS] source-side live call-site exercised for L0_WK diff_count=%u\n",
                (unsigned)live_k_diff_count);
    std::printf("[p11i][PASS] source-side live call-site exercised for L0_WV diff_count=%u\n",
                (unsigned)live_v_diff_count);
    std::printf("[p11i][PASS] live Q and Q_act_q exact-bit match rows=%u\n",
                (unsigned)wq_rec.rows);
    std::printf("[p11i][PASS] live K and K_act_q exact-bit match rows=%u\n",
                (unsigned)wk_rec.rows);
    std::printf("[p11i][PASS] live V and V_act_q exact-bit match rows=%u\n",
                (unsigned)wv_rec.rows);

    // intentional gate violation: shape mismatch should force fallback
    clear_attn_outputs(sram, sc);
    aecct::AttnCfg bad_cfg = cfg;
    bad_cfg.d_model = (aecct::u32_t)31u;
    bad_cfg.d_head = (aecct::u32_t)31u;
    aecct::AttnLayer0<aecct::ATTN_STAGE_QKV>(
        sram,
        bad_cfg,
        (aecct::u32_t)x_in_base,
    ...
    const uint32_t bad_cols = (uint32_t)bad_cfg.d_model.to_uint();
    ...
    std::printf("[p11i][PASS] gate-violation fallback pass d_model=%u expected=%u\n",
                (unsigned)bad_cols,
                (unsigned)wq_rec.cols);

    std::printf("PASS: tb_ternary_live_cut_p11i\n");
    return 0;
}
