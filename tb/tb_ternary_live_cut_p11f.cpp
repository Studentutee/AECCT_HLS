// tb_ternary_live_cut_p11f.cpp
// P00-011F: minimal design-side reusable live ternary helper proof for L0_WQ.
...
static void expect_rsp(aecct::ctrl_ch_t& ctrl_rsp, uint8_t expect_kind, uint8_t expect_payload) {
    ...
    if (kind != expect_kind || payload != expect_payload) {
        std::printf("[p11f][FAIL] rsp mismatch kind=%u payload=%u expect_kind=%u expect_payload=%u\n",
                    (unsigned)kind,
                    (unsigned)payload,
...
static void expect_state(aecct::TopState expect_s) {
    ...
    if ((unsigned)got != (unsigned)expect_s) {
        std::printf("[p11f][FAIL] state mismatch got=%u expect=%u\n",
                    (unsigned)got,
                    (unsigned)expect_s);
        std::exit(1);
    }
...
static void compare_exact_or_die(const char* label, uint32_t got_bits, uint32_t expect_bits, uint32_t idx) {
    if (got_bits != expect_bits) {
        std::printf("[p11f][FAIL] %s mismatch idx=%u got=0x%08X expect=0x%08X got_f=%g expect_f=%g\n",
                    label,
                    (unsigned)idx,
                    (unsigned)got_bits,
...
int main() {
    ...
    const QuantLinearMeta meta = aecct::ternary_linear_live_l0_wq_meta();
    ...
    aecct::u32_t helper_inv_sw_bits = 0;
    if (!aecct::ternary_linear_live_l0_wq_read_inv_sw_bits(sram, (aecct::u32_t)param_base_word, helper_inv_sw_bits)) {
        fail("ternary_linear_live_l0_wq_read_inv_sw_bits returned false");
    }
    compare_exact_or_die("helper inv_s_w bits", (uint32_t)helper_inv_sw_bits.to_uint(), rec.inv_sw_bits, 0u);

    for (uint32_t out = 0u; out < rec.rows; ++out) {
        const uint32_t w_row = out * rec.cols;
        for (uint32_t in = 0u; in < rec.cols; ++in) {
            aecct::quant_w_t decoded_w = 0;
            if (!aecct::ternary_linear_live_l0_wq_decode_weight(
                    sram,
                    (aecct::u32_t)param_base_word,
                    out,
                    in,
                    decoded_w)) {
                fail("ternary_linear_live_l0_wq_decode_weight returned false on valid payload");
            }
            const aecct::quant_w_t expect_w = aecct::quant_w_t((int)w_decoder_layers_0_self_attn_linears_0_weight[w_row + in]);
            if (decoded_w != expect_w) {
                std::printf("[p11f][FAIL] decoded weight mismatch out=%u in=%u got=%g expect=%g\n",
                            (unsigned)out,
                            (unsigned)in,
                            decoded_w.to_double(),
                            expect_w.to_double());
    ...
    const uint32_t saved_word0 = (uint32_t)sram[payload_base + 0u].to_uint();
    sram[payload_base + 0u] = (aecct::u32_t)((saved_word0 & ~0x3u) | 0x2u);
    aecct::quant_w_t illegal_w = 0;
    if (aecct::ternary_linear_live_l0_wq_decode_weight(
            sram,
            (aecct::u32_t)param_base_word,
            0u,
            0u,
            illegal_w)) {
        fail("illegal ternary code was accepted");
    }
    sram[payload_base + 0u] = (aecct::u32_t)saved_word0;
    ...
    for (uint32_t out = 0u; out < rec.rows; ++out) {
        aecct::u32_t got_q_bits = 0;
        aecct::u32_t got_inv_sw_bits = 0;
        ...
        compare_exact_or_die("helper compute inv_s_w bits", (uint32_t)got_inv_sw_bits.to_uint(), rec.inv_sw_bits, out);
        ...
        compare_exact_or_die("helper q elem", (uint32_t)got_q_bits.to_uint(), expect_q_bits, out);
    }

    std::printf("[p11f][PASS] matrix=L0_WQ live load metadata rows=%u cols=%u payload_words_2b=%u last_word_valid_count=%u\n",
                (unsigned)rec.rows,
                (unsigned)rec.cols,
                (unsigned)rec.payload_words_2b,
                (unsigned)rec.last_word_valid_count);
    std::printf("[p11f][PASS] matrix=L0_WQ decoded ternary weights exact match count=%u\n",
                (unsigned)rec.num_weights);
    std::printf("[p11f][PASS] matrix=L0_WQ illegal code rejection verified\n");
    std::printf("[p11f][PASS] matrix=L0_WQ reusable helper q_elem exact-bit match rows=%u\n",
                (unsigned)rec.rows);
    std::printf("PASS: tb_ternary_live_cut_p11f\n");
    return 0;
}
