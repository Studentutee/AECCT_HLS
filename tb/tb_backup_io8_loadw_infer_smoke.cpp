...
#include "RefModel.h"
#include "common/backup_io8_compare_common.h"
...
static const uint32_t kRefRawQuantTraceFocusDims = 3u;
...
struct RefModelStageCompareResult {
    ...
    bool layer1_sublayer1_ln_in_exact;
    bool layer1_sublayer1_ln_affine_out_exact;
    bool layer1_sublayer1_ln_writeback_exact;
    bool end_norm_old_baseline_exact;
    bool st_old_baseline_exact;
    bool logit_old_baseline_exact;
    bool end_norm_exact;
    bool st_exact;
    bool logit_exact;
    bool xpred_exact;
    ...
    uint32_t layer1_sublayer1_ln_in_first_mismatch_token;
    uint32_t layer1_sublayer1_ln_in_first_mismatch_dim;
    uint32_t layer1_sublayer1_ln_in_dut_bits;
    uint32_t layer1_sublayer1_ln_in_ref_bits;
    uint32_t layer1_sublayer1_ln_affine_out_first_mismatch_token;
    ...
    uint32_t layer1_sublayer1_ln_writeback_ref_bits;
    ...
    uint32_t end_norm_old_baseline_mismatch_count;
    uint32_t end_norm_dut_aligned_mismatch_count;
    uint32_t st_old_baseline_mismatch_count;
    uint32_t st_dut_aligned_mismatch_count;
    uint32_t logit_old_baseline_mismatch_count;
    uint32_t logit_dut_aligned_mismatch_count;
...
static RefModelStageCompareResult run_one_ref_model_stage_probe(
    ...
    r.layer1_sublayer1_ln_in_exact = true;
    r.layer1_sublayer1_ln_affine_out_exact = true;
    r.layer1_sublayer1_ln_writeback_exact = true;
    r.end_norm_old_baseline_exact = true;
    r.st_old_baseline_exact = true;
    r.logit_old_baseline_exact = true;
    ...
    r.layer1_sublayer1_ln_in_first_mismatch_token = 0u;
    r.layer1_sublayer1_ln_in_first_mismatch_dim = 0u;
    r.layer1_sublayer1_ln_in_dut_bits = 0u;
    r.layer1_sublayer1_ln_in_ref_bits = 0u;
    r.layer1_sublayer1_ln_affine_out_first_mismatch_token = 0u;
    ...
    r.layer1_sublayer1_ln_writeback_ref_bits = 0u;
    ...
    r.end_norm_old_baseline_mismatch_count = 0u;
    r.end_norm_dut_aligned_mismatch_count = 0u;
    r.st_old_baseline_mismatch_count = 0u;
    r.st_dut_aligned_mismatch_count = 0u;
    r.logit_old_baseline_mismatch_count = 0u;
    r.logit_dut_aligned_mismatch_count = 0u;
    ...
    std::vector<double> ref_layer0_mid_norm_dut_aligned((uint32_t)N_NODES * (uint32_t)D_MODEL, 0.0);
    std::vector<double> ref_layer1_attn_input((uint32_t)N_NODES * (uint32_t)D_MODEL, 0.0);
    std::vector<double> ref_layer1_attn_input_dut_aligned((uint32_t)N_NODES * (uint32_t)D_MODEL, 0.0);
    ...
    std::vector<double> ref_layer1_sublayer1_ln_in_dut_aligned((uint32_t)N_NODES * (uint32_t)D_MODEL, 0.0);
    std::vector<double> ref_layer1_sublayer1_ln_affine_out_dut_aligned((uint32_t)N_NODES * (uint32_t)D_MODEL, 0.0);
    std::vector<double> ref_layer1_sublayer1_ln_out_dut_aligned((uint32_t)N_NODES * (uint32_t)D_MODEL, 0.0);
    ...
    ref_io.out_layer0_mid_norm_dut_aligned = ref_layer0_mid_norm_dut_aligned.data();
    ref_io.out_layer1_attn_input = ref_layer1_attn_input.data();
    ref_io.out_layer1_attn_input_dut_aligned = ref_layer1_attn_input_dut_aligned.data();
    ...
    ref_io.out_layer1_sublayer1_ln_in_dut_aligned = ref_layer1_sublayer1_ln_in_dut_aligned.data();
    ref_io.out_layer1_sublayer1_ln_affine_out_dut_aligned = ref_layer1_sublayer1_ln_affine_out_dut_aligned.data();
    ref_io.out_layer1_sublayer1_ln_out_dut_aligned = ref_layer1_sublayer1_ln_out_dut_aligned.data();
    ...
    const uint32_t final_scalar_base = (uint32_t)sram_map::SCR_FINAL_SCALAR_BASE_W;
    const uint32_t logits_base = (uint32_t)aecct::top_peek_infer_logits_base_word().to_uint();
    const aecct::u32_t top_w_base_word = aecct::top_peek_w_base_word();
    const uint32_t w_base_word = (uint32_t)top_w_base_word.to_uint();
    const aecct::HeadParamBase hp = aecct::make_head_param_base(top_w_base_word);
    const uint32_t ffn1_w_base = (uint32_t)hp.ffn1_w_base_word.to_uint();
    const uint32_t ffn1_b_base = (uint32_t)hp.ffn1_b_base_word.to_uint();
    const uint32_t out_fc_w_base = (uint32_t)hp.out_fc_w_base_word.to_uint();
    const uint32_t out_fc_b_base = (uint32_t)hp.out_fc_b_base_word.to_uint();
    ...
    auto make_same_semantic_cmp_pass = []() -> SameSemanticCmp {
        SameSemanticCmp c;
        c.exact = true;
        c.token = 0u;
        c.dim = 0u;
    ...
    REF_MID_NORM_OUTPUT_WRITEBACK_COMPARE_TOKEN_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
        ...
        REF_MID_NORM_OUTPUT_WRITEBACK_COMPARE_DIM_LOOP: for (uint32_t d = 0u; d < d_model; ++d) {
            ...
            const uint32_t ref_bits = f32_to_bits((float)ref_layer0_mid_norm_dut_aligned[flat]);
            if (dut_bits != ref_bits) {
    ...
    SameSemanticCmp mid_norm_diag_cmp = make_same_semantic_cmp_pass();
    MID_NORM_DIAG_COMPARE_TOKEN_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
        const uint32_t row_base = t * d_model;
        MID_NORM_DIAG_COMPARE_DIM_LOOP: for (uint32_t d = 0u; d < d_model; ++d) {
            const uint32_t flat = row_base + d;
            const uint32_t dut_bits =
                (uint32_t)aecct::top_peek_infer_mid_norm_output_word(flat).to_uint();
            const uint32_t ref_bits = f32_to_bits((float)ref_layer1_attn_input[flat]);
            if (dut_bits != ref_bits) {
                mid_norm_diag_cmp.exact = false;
                mid_norm_diag_cmp.token = t;
                mid_norm_diag_cmp.dim = d;
                mid_norm_diag_cmp.dut_bits = dut_bits;
                mid_norm_diag_cmp.ref_bits = ref_bits;
                break;
            }
        }
        if (!mid_norm_diag_cmp.exact) {
            break;
        }
    }
    ...
    REF_LAYER1_ATTN_INPUT_COMPARE_TOKEN_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
        ...
        REF_LAYER1_ATTN_INPUT_COMPARE_DIM_LOOP: for (uint32_t d = 0u; d < d_model; ++d) {
            ...
            const uint32_t ref_bits = f32_to_bits((float)ref_layer1_attn_input_dut_aligned[flat]);
            if (dut_bits != ref_bits) {
    ...
    SameSemanticCmp layer1_attn_input_diag_cmp = make_same_semantic_cmp_pass();
    LAYER1_ATTN_INPUT_DIAG_COMPARE_TOKEN_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
        const uint32_t row_base = t * d_model;
        LAYER1_ATTN_INPUT_DIAG_COMPARE_DIM_LOOP: for (uint32_t d = 0u; d < d_model; ++d) {
            const uint32_t flat = row_base + d;
            const uint32_t dut_bits = (flat < layer1_x_words) ?
                (uint32_t)aecct::transformer_layer_debug_peek_layer1_attn_input_word((aecct::u32_t)flat).to_uint() : 0u;
            const uint32_t ref_bits = f32_to_bits((float)ref_layer1_attn_input[flat]);
            if (dut_bits != ref_bits) {
                layer1_attn_input_diag_cmp.exact = false;
                layer1_attn_input_diag_cmp.token = t;
                layer1_attn_input_diag_cmp.dim = d;
                layer1_attn_input_diag_cmp.dut_bits = dut_bits;
                layer1_attn_input_diag_cmp.ref_bits = ref_bits;
                break;
            }
        }
        if (!layer1_attn_input_diag_cmp.exact) {
            break;
        }
    }
    std::printf(
        "[backup_io8][mid_norm_same_semantic] sample=%u authoritative_target=layer0_mid_norm_dut_aligned_out authoritative_exact=%u diagnostic_target=layer1_attn_input diagnostic_only_exact=%u first_mismatch_token=%u first_mismatch_dim=%u dut=0x%08X ref=0x%08X\n",
        (unsigned)sample_idx,
        (unsigned)(r.mid_norm_output_writeback_exact ? 1u : 0u),
        (unsigned)(mid_norm_diag_cmp.exact ? 1u : 0u),
    ...
    std::printf(
        "[backup_io8][layer1_attn_input_same_semantic] sample=%u authoritative_target=layer1_attn_input_dut_aligned authoritative_exact=%u diagnostic_target=layer1_attn_input diagnostic_only_exact=%u first_mismatch_token=%u first_mismatch_dim=%u dut=0x%08X ref=0x%08X\n",
        (unsigned)sample_idx,
        (unsigned)(r.layer1_attn_input_readback_exact ? 1u : 0u),
        (unsigned)(layer1_attn_input_diag_cmp.exact ? 1u : 0u),
    ...
    REF_LAYER1_SUBLAYER1_LN_IN_COMPARE_TOKEN_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
        const uint32_t row_base = t * d_model;
        REF_LAYER1_SUBLAYER1_LN_IN_COMPARE_DIM_LOOP: for (uint32_t d = 0u; d < d_model; ++d) {
            const uint32_t flat = row_base + d;
            ...
            const uint32_t dut_bits = f32_to_bits(dut_ffn2 + dut_ln0);
            const uint32_t ref_bits = f32_to_bits((float)ref_layer1_sublayer1_ln_in_dut_aligned[flat]);
            if (dut_bits != ref_bits) {
                r.layer1_sublayer1_ln_in_exact = false;
                r.layer1_sublayer1_ln_in_first_mismatch_token = t;
                r.layer1_sublayer1_ln_in_first_mismatch_dim = d;
                r.layer1_sublayer1_ln_in_dut_bits = dut_bits;
                r.layer1_sublayer1_ln_in_ref_bits = ref_bits;
                break;
            }
        }
        if (!r.layer1_sublayer1_ln_in_exact) {
            break;
        }
    }
    ...
    REF_LAYER1_OUT_COMPARE_TOKEN_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
        ...
        REF_LAYER1_OUT_COMPARE_DIM_LOOP: for (uint32_t d = 0u; d < d_model; ++d) {
            ...
            const uint32_t ref_bits = f32_to_bits((float)ref_layer1_sublayer1_ln_out_dut_aligned[row_base + d]);
            if (dut_bits != ref_bits) {
                ...
                r.layer1_sublayer1_ln_writeback_exact = false;
                r.layer1_sublayer1_ln_writeback_first_mismatch_token = t;
                r.layer1_sublayer1_ln_writeback_first_mismatch_dim = d;
                r.layer1_sublayer1_ln_writeback_dut_bits = dut_bits;
                r.layer1_sublayer1_ln_writeback_ref_bits = ref_bits;
                break;
            }
        }
    ...
    SameSemanticCmp layer1_sublayer1_ln_diag_cmp = make_same_semantic_cmp_pass();
    LAYER1_SUBLAYER1_LN_DIAG_COMPARE_TOKEN_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
        const uint32_t row_base = t * d_model;
        LAYER1_SUBLAYER1_LN_DIAG_COMPARE_DIM_LOOP: for (uint32_t d = 0u; d < d_model; ++d) {
            const uint32_t flat = row_base + d;
            const uint32_t dut_bits =
                (uint32_t)aecct::top_peek_infer_endln_input_word(flat).to_uint();
            const uint32_t ref_bits = f32_to_bits((float)ref_layer1_ffn_ln_out[flat]);
            if (dut_bits != ref_bits) {
                layer1_sublayer1_ln_diag_cmp.exact = false;
                layer1_sublayer1_ln_diag_cmp.token = t;
                layer1_sublayer1_ln_diag_cmp.dim = d;
                layer1_sublayer1_ln_diag_cmp.dut_bits = dut_bits;
                layer1_sublayer1_ln_diag_cmp.ref_bits = ref_bits;
                break;
            }
        }
        if (!layer1_sublayer1_ln_diag_cmp.exact) {
            break;
        }
    }
    if (!r.layer1_sublayer1_ln_in_exact) {
        r.layer1_sublayer1_ln_bucket = 1u;
    } else if (!r.layer1_sublayer1_ln_affine_out_exact) {
        r.layer1_sublayer1_ln_bucket = 2u;
    } else if (!r.layer1_sublayer1_ln_writeback_exact) {
        r.layer1_sublayer1_ln_bucket = 3u;
    } else {
        r.layer1_sublayer1_ln_bucket = 0u;
    }
    const char* layer1_sublayer1_ln_first_divergence = "none";
    uint32_t layer1_sublayer1_ln_first_token = 0u;
    uint32_t layer1_sublayer1_ln_first_dim = 0u;
    uint32_t layer1_sublayer1_ln_first_dut_bits = 0u;
    uint32_t layer1_sublayer1_ln_first_ref_bits = 0u;
    if (r.layer1_sublayer1_ln_bucket == 1u) {
        layer1_sublayer1_ln_first_divergence = "layer1_sublayer1_ln_in";
        layer1_sublayer1_ln_first_token = r.layer1_sublayer1_ln_in_first_mismatch_token;
        layer1_sublayer1_ln_first_dim = r.layer1_sublayer1_ln_in_first_mismatch_dim;
        layer1_sublayer1_ln_first_dut_bits = r.layer1_sublayer1_ln_in_dut_bits;
        layer1_sublayer1_ln_first_ref_bits = r.layer1_sublayer1_ln_in_ref_bits;
    } else if (r.layer1_sublayer1_ln_bucket == 2u) {
        layer1_sublayer1_ln_first_divergence = "layer1_sublayer1_ln_affine_out";
        layer1_sublayer1_ln_first_token = r.layer1_sublayer1_ln_affine_out_first_mismatch_token;
        layer1_sublayer1_ln_first_dim = r.layer1_sublayer1_ln_affine_out_first_mismatch_dim;
        layer1_sublayer1_ln_first_dut_bits = r.layer1_sublayer1_ln_affine_out_dut_bits;
        layer1_sublayer1_ln_first_ref_bits = r.layer1_sublayer1_ln_affine_out_ref_bits;
    } else if (r.layer1_sublayer1_ln_bucket == 3u) {
        layer1_sublayer1_ln_first_divergence = "layer1_sublayer1_ln_writeback";
        layer1_sublayer1_ln_first_token = r.layer1_sublayer1_ln_writeback_first_mismatch_token;
        layer1_sublayer1_ln_first_dim = r.layer1_sublayer1_ln_writeback_first_mismatch_dim;
    ...
    std::printf(
        ...
        (unsigned)sample_idx,
        (unsigned)(r.layer1_sublayer1_ln_writeback_exact ? 1u : 0u),
        (unsigned)(layer1_sublayer1_ln_diag_cmp.exact ? 1u : 0u),
    ...
    std::printf(
        ...
        (unsigned)(r.layer1_sublayer1_ln_writeback_exact ? 1u : 0u),
        layer1_sublayer1_ln_first_divergence,
        (unsigned)layer1_sublayer1_ln_first_token,
        (unsigned)layer1_sublayer1_ln_first_dim,
        (unsigned)layer1_sublayer1_ln_first_dut_bits,
        (unsigned)layer1_sublayer1_ln_first_ref_bits,
    ...
    const ac_int<32, true> end_d_model_i = (ac_int<32, true>)((d_model == 0u) ? 1u : d_model);
    const aecct::fp32_t end_d_model_den(end_d_model_i);
    const aecct::fp32_t end_ln_eps_fp = aecct::fp32_from_bits(aecct::LN_EPS_BITS);
    const uint32_t end_norm_gamma_base = w_base_word + kParamMeta[64u].offset_w;
    const uint32_t end_norm_beta_base = w_base_word + kParamMeta[16u].offset_w;
    std::vector<uint32_t> end_norm_dut_aligned_words((uint32_t)N_NODES * d_model, 0u);
    backup_io8_compare_common::DualCompareSummary end_norm_dual_cmp =
        backup_io8_compare_common::make_dual_compare_summary();
    bool end_norm_old_first_mismatch_valid = false;
    uint32_t end_norm_old_first_mismatch_token = 0u;
    uint32_t end_norm_old_first_mismatch_dim = 0u;
    uint32_t end_norm_old_first_mismatch_dut_bits = 0u;
    uint32_t end_norm_old_first_mismatch_ref_bits = 0u;
    END_NORM_DUT_ALIGNED_TOKEN_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
        const uint32_t row_base = t * d_model;
        aecct::fp32_t sum_fp = aecct::fp32_zero();
        END_NORM_DUT_ALIGNED_SUM_LOOP: for (uint32_t d = 0u; d < d_model; ++d) {
            const uint32_t ln_in_bits =
                (uint32_t)aecct::top_peek_infer_endln_input_word(row_base + d).to_uint();
            const aecct::fp32_t ln_in_raw = tb_fp32_from_u32_bits(ln_in_bits);
            const aecct::fp32_t ln_in_sanitized = aecct::layernorm_fp32_sanitize_input(ln_in_raw);
            sum_fp += ln_in_sanitized;
        }
        const aecct::fp32_t mean_fp = sum_fp / end_d_model_den;
        aecct::fp32_t var_acc_fp = aecct::fp32_zero();
        END_NORM_DUT_ALIGNED_VAR_LOOP: for (uint32_t d = 0u; d < d_model; ++d) {
            const uint32_t ln_in_bits =
                (uint32_t)aecct::top_peek_infer_endln_input_word(row_base + d).to_uint();
            const aecct::fp32_t ln_in_raw = tb_fp32_from_u32_bits(ln_in_bits);
            const aecct::fp32_t ln_in_sanitized = aecct::layernorm_fp32_sanitize_input(ln_in_raw);
            const aecct::fp32_t delta = ln_in_sanitized - mean_fp;
            var_acc_fp += (delta * delta);
        }
        const aecct::fp32_t var_fp = var_acc_fp / end_d_model_den;
        aecct::fp32_t var_plus_eps_fp = var_fp + end_ln_eps_fp;
        if (!aecct::layernorm_fp32_is_finite(var_plus_eps_fp) || var_plus_eps_fp <= aecct::fp32_zero()) {
            var_plus_eps_fp = end_ln_eps_fp;
        }
        if (!aecct::layernorm_fp32_is_finite(var_plus_eps_fp) || var_plus_eps_fp <= aecct::fp32_zero()) {
            var_plus_eps_fp = aecct::fp32_one();
        }
        const aecct::fp32_t inv_std_fp = aecct::layernorm_refstyle_inv_sqrt_approx(var_plus_eps_fp);
        END_NORM_DUT_ALIGNED_AFFINE_LOOP: for (uint32_t d = 0u; d < d_model; ++d) {
            const uint32_t flat = row_base + d;
            const uint32_t ln_in_bits =
                (uint32_t)aecct::top_peek_infer_endln_input_word(flat).to_uint();
            const aecct::fp32_t ln_in_raw = tb_fp32_from_u32_bits(ln_in_bits);
            const aecct::fp32_t ln_in_sanitized = aecct::layernorm_fp32_sanitize_input(ln_in_raw);
            const aecct::fp32_t gamma_fp =
                tb_fp32_from_u32_bits((uint32_t)sram[end_norm_gamma_base + d].to_uint());
            const aecct::fp32_t beta_fp =
                tb_fp32_from_u32_bits((uint32_t)sram[end_norm_beta_base + d].to_uint());
            const aecct::fp32_t y_fp = ((ln_in_sanitized - mean_fp) * inv_std_fp) * gamma_fp + beta_fp;
            const uint32_t dut_aligned_bits = tb_u32_bits_from_fp32(y_fp);
            end_norm_dut_aligned_words[flat] = dut_aligned_bits;

            const uint32_t dut_bits = (uint32_t)sram[x_end_base + flat].to_uint();
            const uint32_t old_ref_bits = f32_to_bits((float)ref_end_norm[flat]);
            if (!end_norm_old_first_mismatch_valid && dut_bits != old_ref_bits) {
                end_norm_old_first_mismatch_valid = true;
                end_norm_old_first_mismatch_token = t;
                end_norm_old_first_mismatch_dim = d;
                end_norm_old_first_mismatch_dut_bits = dut_bits;
                end_norm_old_first_mismatch_ref_bits = old_ref_bits;
            }
            backup_io8_compare_common::dual_compare_update(
                end_norm_dual_cmp,
                dut_bits == old_ref_bits,
                dut_bits == dut_aligned_bits
            );
            if (r.end_norm_exact && dut_bits != dut_aligned_bits) {
                r.end_norm_exact = false;
                r.end_norm_first_mismatch_token = t;
                r.end_norm_first_mismatch_dim = d;
                r.end_norm_dut_bits = dut_bits;
                r.end_norm_ref_bits = dut_aligned_bits;
            }
        }
    }
    r.end_norm_old_baseline_mismatch_count = end_norm_dual_cmp.old_baseline_mismatch_count;
    r.end_norm_dut_aligned_mismatch_count = end_norm_dual_cmp.dut_aligned_mismatch_count;
    r.end_norm_old_baseline_exact = backup_io8_compare_common::dual_compare_old_exact(end_norm_dual_cmp);
    r.end_norm_exact = backup_io8_compare_common::dual_compare_aligned_exact(end_norm_dual_cmp);
    if (r.end_norm_exact) {
        r.end_norm_dut_bits = (uint32_t)sram[x_end_base + focused_idx * d_model].to_uint();
        r.end_norm_ref_bits = end_norm_dut_aligned_words[focused_idx * d_model];
    }

    std::vector<uint32_t> st_dut_aligned_words((uint32_t)N_NODES, 0u);
    backup_io8_compare_common::DualCompareSummary st_dual_cmp =
        backup_io8_compare_common::make_dual_compare_summary();
    ST_DUT_ALIGNED_GEN_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
        const uint32_t row_base = t * d_model;
        aecct::fp32_t st_acc_fp = tb_fp32_from_u32_bits((uint32_t)sram[ffn1_b_base + 0u].to_uint());
        ST_DUT_ALIGNED_DOT_LOOP: for (uint32_t i = 0u; i < d_model; ++i) {
            const aecct::fp32_t xv_fp = tb_fp32_from_u32_bits(end_norm_dut_aligned_words[row_base + i]);
            const aecct::fp32_t wv_fp = tb_fp32_from_u32_bits((uint32_t)sram[ffn1_w_base + i].to_uint());
            st_acc_fp += (xv_fp * wv_fp);
        }
        st_dut_aligned_words[t] = tb_u32_bits_from_fp32(st_acc_fp);
    }
    REF_ST_DUAL_COMPARE_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
        const uint32_t dut_bits = (uint32_t)sram[final_scalar_base + t].to_uint();
        const uint32_t old_ref_bits = f32_to_bits((float)ref_st[t]);
        const uint32_t dut_aligned_bits = st_dut_aligned_words[t];
        backup_io8_compare_common::dual_compare_update(
            st_dual_cmp,
            dut_bits == old_ref_bits,
            dut_bits == dut_aligned_bits
        );
        if (r.st_exact && dut_bits != dut_aligned_bits) {
            r.st_exact = false;
            r.st_first_mismatch_idx = t;
            r.st_dut_bits = dut_bits;
            r.st_ref_bits = dut_aligned_bits;
        }
    }
    r.st_old_baseline_mismatch_count = st_dual_cmp.old_baseline_mismatch_count;
    r.st_dut_aligned_mismatch_count = st_dual_cmp.dut_aligned_mismatch_count;
    r.st_old_baseline_exact = backup_io8_compare_common::dual_compare_old_exact(st_dual_cmp);
    r.st_exact = backup_io8_compare_common::dual_compare_aligned_exact(st_dual_cmp);
    if (r.st_exact) {
        r.st_dut_bits = (uint32_t)sram[final_scalar_base + focused_idx].to_uint();
        r.st_ref_bits = st_dut_aligned_words[focused_idx];
    }

    std::vector<uint32_t> logits_dut_aligned_words((uint32_t)EXP_LEN_OUT_LOGITS_WORDS, 0u);
    backup_io8_compare_common::DualCompareSummary logit_dual_cmp =
        backup_io8_compare_common::make_dual_compare_summary();
    LOGIT_DUT_ALIGNED_GEN_LOOP: for (uint32_t c = 0u; c < (uint32_t)EXP_LEN_OUT_LOGITS_WORDS; ++c) {
        aecct::fp32_t acc_fp = tb_fp32_from_u32_bits((uint32_t)sram[out_fc_b_base + c].to_uint());
        LOGIT_DUT_ALIGNED_TOKEN_REDUCE_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
            const aecct::fp32_t st_fp = tb_fp32_from_u32_bits(st_dut_aligned_words[t]);
            const aecct::fp32_t w_fp =
                tb_fp32_from_u32_bits((uint32_t)sram[out_fc_w_base + c * (uint32_t)N_NODES + t].to_uint());
            acc_fp += (w_fp * st_fp);
        }
        logits_dut_aligned_words[c] = tb_u32_bits_from_fp32(acc_fp);
    }
    REF_LOGIT_DUAL_COMPARE_LOOP: for (uint32_t i = 0u; i < (uint32_t)EXP_LEN_OUT_LOGITS_WORDS; ++i) {
        const uint32_t dut_bits = (uint32_t)sram[logits_base + i].to_uint();
        const uint32_t old_ref_bits = f32_to_bits((float)ref_logits[i]);
        const uint32_t dut_aligned_bits = logits_dut_aligned_words[i];
        backup_io8_compare_common::dual_compare_update(
            logit_dual_cmp,
            dut_bits == old_ref_bits,
            dut_bits == dut_aligned_bits
        );
        if (r.logit_exact && dut_bits != dut_aligned_bits) {
            r.logit_exact = false;
            r.logit_first_mismatch_idx = i;
            r.logit_dut_bits = dut_bits;
            r.logit_ref_bits = dut_aligned_bits;
        }
    }
    r.logit_old_baseline_mismatch_count = logit_dual_cmp.old_baseline_mismatch_count;
    r.logit_dut_aligned_mismatch_count = logit_dual_cmp.dut_aligned_mismatch_count;
    r.logit_old_baseline_exact = backup_io8_compare_common::dual_compare_old_exact(logit_dual_cmp);
    r.logit_exact = backup_io8_compare_common::dual_compare_aligned_exact(logit_dual_cmp);
    if (r.logit_exact) {
    ...
    if (is_w2_semantic_scan_sample(sample_idx)) {
        ...
        auto make_raw_trace_cmp_pass = []() -> RawTraceCmp {
            RawTraceCmp c;
            c.exact = true;
            c.token = 0u;
            c.dim = 0u;
        ...
        auto mark_raw_trace_cmp = [&](RawTraceCmp& c,
                                      uint32_t token,
                                      uint32_t dim,
                                      uint32_t index,
                                      uint32_t dut_bits,
        ...
        W2_RAW_TRACE_QX_TOKEN_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
            const uint32_t ffn_row = t * d_ffn;
            W2_RAW_TRACE_QX_COL_LOOP: for (uint32_t c = 0u; c < d_ffn; ++c) {
                const uint32_t ref_input_bits = f32_to_bits((float)ref_layer0_relu_out[ffn_row + c]);
                const aecct::fp32_t x_fp = aecct::fp32_from_bits((aecct::u32_t)ref_input_bits);
                const aecct::fp32_t qx_fp = aecct::ffn_quantize_int8_symmetric(x_fp, layer0_w2_sx_fp);
                const uint32_t dut_qx_bits = (uint32_t)aecct::bits_from_fp32(qx_fp).to_uint();
                const uint32_t ref_qx_bits = f32_to_bits((float)ref_layer0_ffn_w2_quant_raw_qx[ffn_row + c]);
                if (dut_qx_bits != ref_qx_bits) {
                    mark_raw_trace_cmp(qx_cmp, t, 0u, c, dut_qx_bits, ref_qx_bits);
                    break;
                }
            }
            if (!qx_cmp.exact) {
        ...
        W2_RAW_TRACE_BIAS_DIM_LOOP: for (uint32_t d = 0u; d < d_model; ++d) {
            const uint32_t dut_bias_bits = (uint32_t)sram[layer0_w2_bias_base + d].to_uint();
            const uint32_t ref_bias_bits = f32_to_bits((float)ref_layer0_ffn_w2_quant_raw_bias_domain[d]);
            if (dut_bias_bits != ref_bias_bits) {
                mark_raw_trace_cmp(bias_domain_cmp, 0u, d, 0u, dut_bias_bits, ref_bias_bits);
                break;
            }
        }

        W2_RAW_TRACE_PARTIAL_TOKEN_LOOP: for (uint32_t t = 0u; t < (uint32_t)N_NODES; ++t) {
            const uint32_t ffn_row = t * d_ffn;
            W2_RAW_TRACE_PARTIAL_FOCUS_DIM_LOOP: for (uint32_t fd = 0u; fd < kRefRawQuantTraceFocusDims && fd < d_model; ++fd) {
                const uint32_t d = fd;
                const uint32_t dut_bias_bits = (uint32_t)sram[layer0_w2_bias_base + d].to_uint();
                aecct::fp32_t dut_acc_fp = aecct::fp32_from_bits((aecct::u32_t)dut_bias_bits);
                const uint32_t ref_partial_base = (t * kRefRawQuantTraceFocusDims + fd) * partial_acc_steps;
                const uint32_t dut_step0_bits = (uint32_t)aecct::bits_from_fp32(dut_acc_fp).to_uint();
                const uint32_t ref_step0_bits = f32_to_bits((float)ref_layer0_ffn_w2_quant_raw_partial_acc_focus[ref_partial_base]);
                if (dut_step0_bits != ref_step0_bits) {
                    mark_raw_trace_cmp(partial_acc_cmp, t, d, 0u, dut_step0_bits, ref_step0_bits);
                    break;
                }
                W2_RAW_TRACE_PARTIAL_COL_LOOP: for (uint32_t c = 0u; c < d_ffn; ++c) {
                    const uint32_t ref_input_bits = f32_to_bits((float)ref_layer0_relu_out[ffn_row + c]);
                    const aecct::fp32_t x_fp = aecct::fp32_from_bits((aecct::u32_t)ref_input_bits);
                    const aecct::fp32_t qx_fp = aecct::ffn_quantize_int8_symmetric(x_fp, layer0_w2_sx_fp);
                    const uint32_t ref_weight_bits = (uint32_t)sram[layer0_w2_weight_base + d * d_ffn + c].to_uint();
                    const aecct::fp32_t w_fp = aecct::fp32_from_bits((aecct::u32_t)ref_weight_bits);
                    dut_acc_fp += qx_fp * (w_fp * layer0_w2_inv_scale_fp);
                    const uint32_t dut_partial_bits = (uint32_t)aecct::bits_from_fp32(dut_acc_fp).to_uint();
                    const uint32_t ref_partial_bits =
                        f32_to_bits((float)ref_layer0_ffn_w2_quant_raw_partial_acc_focus[ref_partial_base + (c + 1u)]);
                    if (dut_partial_bits != ref_partial_bits) {
                        mark_raw_trace_cmp(partial_acc_cmp, t, d, c + 1u, dut_partial_bits, ref_partial_bits);
                        break;
                    }
                }
                if (!partial_acc_cmp.exact) {
                    break;
                }
            }
            if (!partial_acc_cmp.exact) {
        ...
        if (!inv_bits_exact) {
            ...
            raw_quant_first_divergence = "bias_domain";
        } else if (!partial_acc_cmp.exact) {
            raw_quant_first_divergence = "partial_acc";
        } else if (!final_raw_exact) {
            raw_quant_first_divergence = "final_raw_out";
        }
        ...
        if (!weight_scaled_cmp.exact) {
            std::printf(
                "[backup_io8][refmodel_raw_quant_trace][weight_scaled] sample=%u first_mismatch_dim=%u input_col=%u dut=0x%08X ref=0x%08X\n",
                (unsigned)sample_idx,
                (unsigned)weight_scaled_cmp.dim,
                (unsigned)weight_scaled_cmp.index,
                (unsigned)weight_scaled_cmp.dut_bits,
                (unsigned)weight_scaled_cmp.ref_bits);
        }
        if (!bias_domain_cmp.exact) {
            std::printf(
                "[backup_io8][refmodel_raw_quant_trace][bias_domain] sample=%u first_mismatch_dim=%u dut=0x%08X ref=0x%08X\n",
                (unsigned)sample_idx,
                (unsigned)bias_domain_cmp.dim,
                (unsigned)bias_domain_cmp.dut_bits,
                (unsigned)bias_domain_cmp.ref_bits);
        }
        if (!partial_acc_cmp.exact) {
            std::printf(
                "[backup_io8][refmodel_raw_quant_trace][partial_acc] sample=%u first_mismatch_token=%u dim=%u step=%u dut=0x%08X ref=0x%08X\n",
                (unsigned)sample_idx,
                (unsigned)partial_acc_cmp.token,
                (unsigned)partial_acc_cmp.dim,
                (unsigned)partial_acc_cmp.index,
                (unsigned)partial_acc_cmp.dut_bits,
                (unsigned)partial_acc_cmp.ref_bits);
        }
        if (!final_raw_exact) {
            std::printf(
                "[backup_io8][refmodel_raw_quant_trace][final_raw_out] sample=%u first_mismatch_token=%u dim=%u dut=0x%08X ref=0x%08X\n",
                (unsigned)sample_idx,
                (unsigned)w2_scan_same_semantic_cmp.token,
                (unsigned)w2_scan_same_semantic_cmp.dim,
                (unsigned)w2_scan_same_semantic_cmp.dut_bits,
    ...
    r.all_exact =
        ...
        r.layer1_ffn2_exact &&
        r.layer1_sublayer1_ln_in_exact &&
        r.layer1_sublayer1_ln_affine_out_exact &&
        r.layer1_sublayer1_ln_writeback_exact &&
    ...
    } else if (r.layer1_sublayer1_ln_bucket != 0u) {
    ...
    const uint32_t focused_ref_st = st_dut_aligned_words[focused_idx];
    const uint32_t focused_ref_st_old = f32_to_bits((float)ref_st[focused_idx]);
    const uint32_t focused_dut_logit = (uint32_t)sram[logits_base + focused_idx].to_uint();
    const uint32_t focused_ref_logit = logits_dut_aligned_words[focused_idx];
    const uint32_t focused_ref_logit_old = f32_to_bits((float)ref_logits[focused_idx]);
    ...
    const uint32_t focused_ref_mid_norm_output_writeback_bits =
        f32_to_bits((float)ref_layer0_mid_norm_dut_aligned[focused_idx * d_model + focused_end_d]);
    ...
    const uint32_t focused_ref_layer1_attn_input_bits =
        f32_to_bits((float)ref_layer1_attn_input_dut_aligned[focused_idx * d_model + focused_end_d]);
    ...
    const uint32_t focused_ref_layer1_bits =
        f32_to_bits((float)ref_layer1_sublayer1_ln_out_dut_aligned[focused_idx * d_model + focused_end_d]);
    const uint32_t focused_dut_end_norm_bits =
        (uint32_t)sram[x_end_base + focused_idx * d_model + focused_end_d].to_uint();
    const uint32_t focused_ref_end_norm_old_bits =
    ...
    std::printf(
        ...
        (unsigned)focused_ref_end_norm_old_bits,
        (unsigned)(focused_dut_end_norm_bits == focused_ref_end_norm_old_bits ? 1u : 0u));
    backup_io8_compare_common::print_dual_compare_summary(
        "end_norm_dual_compare",
        sample_idx,
        end_norm_dual_cmp
    );
    if (end_norm_old_first_mismatch_valid) {
        std::printf(
            "[backup_io8][ref_model_end_norm][diagnostic_old] sample=%u exact=0 first_mismatch_token=%u dim=%u dut=0x%08X ref_model=0x%08X\n",
            (unsigned)sample_idx,
            (unsigned)end_norm_old_first_mismatch_token,
            (unsigned)end_norm_old_first_mismatch_dim,
            (unsigned)end_norm_old_first_mismatch_dut_bits,
            (unsigned)end_norm_old_first_mismatch_ref_bits);
    } else {
        std::printf(
            "[backup_io8][ref_model_end_norm][diagnostic_old] sample=%u exact=1\n",
            (unsigned)sample_idx);
    }
    if (!r.end_norm_exact) {
        std::printf(
            "[backup_io8][ref_model_end_norm][authoritative] sample=%u exact=0 first_mismatch_token=%u dim=%u dut=0x%08X dut_aligned_ref=0x%08X\n",
            (unsigned)sample_idx,
        ...
        std::printf(
            "[backup_io8][ref_model_end_norm][authoritative] sample=%u exact=1\n",
            (unsigned)sample_idx);
    }
    backup_io8_compare_common::print_dual_compare_summary("st_dual_compare", sample_idx, st_dual_cmp);
    backup_io8_compare_common::print_dual_compare_summary("logit_dual_compare", sample_idx, logit_dual_cmp);
    std::printf(
        "[backup_io8][ref_model_probe] sample=%u idx=%u dut_st=0x%08X ref_model_st=0x%08X ref_model_st_old=0x%08X dut_logit=0x%08X ref_model_logit=0x%08X ref_model_logit_old=0x%08X trace_logit=0x%08X dut_xpred=0x%08X ref_model_xpred=0x%08X trace_xpred=0x%08X\n",
        (unsigned)sample_idx,
        (unsigned)focused_idx,
        (unsigned)focused_dut_st,
        (unsigned)focused_ref_st,
        (unsigned)focused_ref_st_old,
        (unsigned)focused_dut_logit,
        (unsigned)focused_ref_logit,
        (unsigned)focused_ref_logit_old,
        (unsigned)focused_trace_logit,
        (unsigned)focused_dut_xpred,
        (unsigned)focused_ref_xpred,
        (unsigned)focused_trace_xpred);
    std::printf(
        "[backup_io8][ref_model_probe] sample=%u layer1_attn_input_exact=%u layer1_post_concat_exact=%u layer1_q_exact=%u layer1_attn_out_exact=%u layer1_pre_ln_input_exact=%u layer1_ln_out_exact=%u layer1_ffn1_out_exact=%u layer1_relu_out_exact=%u layer1_ffn2_exact=%u layer1_ffn_ln_out_exact=%u end_norm_exact=%u st_exact=%u logit_exact=%u xpred_exact=%u end_norm_old_exact=%u st_old_exact=%u logit_old_exact=%u first_st_mismatch_idx=%u first_logit_mismatch_idx=%u first_xpred_mismatch_idx=%u boundary_class=%u\n",
        (unsigned)sample_idx,
        ...
        (unsigned)(r.end_norm_old_baseline_exact ? 1u : 0u),
        (unsigned)(r.st_old_baseline_exact ? 1u : 0u),
        (unsigned)(r.logit_old_baseline_exact ? 1u : 0u),
    ...
    if (!r.all_exact) {
        if (r.boundary_bucket == 0u) {
            ...
            if (r.layer1_sublayer1_ln_bucket == 1u) {
                std::printf(
                    "[backup_io8][ref_model_probe] first_divergence=layer1_sublayer1_ln_in token=%u dim=%u dut=0x%08X ref=0x%08X\n",
                    (unsigned)r.layer1_sublayer1_ln_in_first_mismatch_token,
                    (unsigned)r.layer1_sublayer1_ln_in_first_mismatch_dim,
                    (unsigned)r.layer1_sublayer1_ln_in_dut_bits,
                    (unsigned)r.layer1_sublayer1_ln_in_ref_bits);
            } else if (r.layer1_sublayer1_ln_bucket == 2u) {
                std::printf(
                    "[backup_io8][ref_model_probe] first_divergence=layer1_sublayer1_ln_affine_out token=%u dim=%u dut=0x%08X ref=0x%08X\n",
                    (unsigned)r.layer1_sublayer1_ln_affine_out_first_mismatch_token,
                    (unsigned)r.layer1_sublayer1_ln_affine_out_first_mismatch_dim,
                    (unsigned)r.layer1_sublayer1_ln_affine_out_dut_bits,
                    (unsigned)r.layer1_sublayer1_ln_affine_out_ref_bits);
            } else if (r.layer1_sublayer1_ln_bucket == 3u) {
                std::printf(
                    "[backup_io8][ref_model_probe] first_divergence=layer1_sublayer1_ln_writeback token=%u dim=%u dut=0x%08X ref=0x%08X\n",
                    (unsigned)r.layer1_sublayer1_ln_writeback_first_mismatch_token,
                    (unsigned)r.layer1_sublayer1_ln_writeback_first_mismatch_dim,
                    (unsigned)r.layer1_sublayer1_ln_writeback_dut_bits,
                    (unsigned)r.layer1_sublayer1_ln_writeback_ref_bits);
            } else {
                std::printf(
...
int main() {
    ...
    W2_DIRECT_PROBE_SAMPLE_LOOP: for (uint32_t i = 0u; i < kW2DirectProbeSampleCount; ++i) {
        ...
        std::printf(
            ...
            first_subbucket);
        backup_io8_compare_common::print_fixed_sample_summary(
            sample_id,
            w2_probe.boundary_bucket,
            w2_probe.end_norm_old_baseline_mismatch_count,
            w2_probe.end_norm_dut_aligned_mismatch_count,
            w2_probe.st_old_baseline_mismatch_count,
            w2_probe.st_dut_aligned_mismatch_count,
            w2_probe.logit_old_baseline_mismatch_count,
            w2_probe.logit_dut_aligned_mismatch_count,
            w2_probe.end_norm_exact,
            w2_probe.st_exact,
            w2_probe.logit_exact,
            w2_probe.xpred_exact
        );
    }
