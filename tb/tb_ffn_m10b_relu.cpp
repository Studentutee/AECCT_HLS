...
int main() {
    ...
    const uint32_t relu_words = (uint32_t)aecct::FFN_W1_OUT_WORDS;
    ...
    aecct::FFNLayer0<aecct::FFN_STAGE_W1>(sram, cfg, (aecct::u32_t)aecct::FFN_X_IN_BASE_WORD_DEFAULT, sc);
    aecct::FFNLayer0<aecct::FFN_STAGE_RELU>(sram, cfg, (aecct::u32_t)aecct::FFN_X_IN_BASE_WORD_DEFAULT, sc);

    int rc = compare_tensor(
        "ffn_relu_out",
        sram,
        (uint32_t)sc.relu_out_base_word.to_uint(),
        trace_layer0_ffn_relu_out_step0_tensor,
        sample_idx,
        relu_words,
        1.0e-5
    );
    ...
    if (rc != 0) {
        std::printf("ERROR: tb_ffn_m10b_relu failed\n");
        return 1;
    }

    std::printf("PASS: tb_ffn_m10b_relu\n");
    return 0;
}
