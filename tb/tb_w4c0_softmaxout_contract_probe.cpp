// W4-C0: SoftmaxOut contract-first mixed bridge probe (local-only).
...
private:
    ...
    bool init_and_bootstrap() {
        ...
        if (!p11aeaf_tb::prepare_qkv_payload_set(payloads_)) {
            std::printf("[w4c0][FAIL] payload preparation failed\n");
            return false;
        }
        ...
        if (!p11aeaf_tb::run_ac_ad_mainline(sram_bootstrap_, q_fallback_taken, kv_fallback_taken)) {
            std::printf("[w4c0][FAIL] AC/AD bootstrap failed\n");
            return false;
        }
        if (q_fallback_taken || kv_fallback_taken) {
            std::printf("[w4c0][FAIL] AC/AD bootstrap fallback detected\n");
            return false;
        }
        ...
        if (!score_mainline_taken || score_fallback_taken) {
            std::printf("[w4c0][FAIL] AE bootstrap failed\n");
            return false;
        }
        ...
        if (candidate_family_cases > kMaxFamilyCases) {
            std::printf("[w4c0][FAIL] family case count overflow count=%u max=%u\n",
        ...
        if (single_valid_words_ == 0u || single_valid_words_ > tile_words) {
            std::printf("[w4c0][FAIL] invalid single tile valid words\n");
            return false;
        }

        family_case_count_ = candidate_family_cases;
        family_total_words_ = 0u;
        for (uint32_t c = 0u; c < family_case_count_; ++c) {
            ...
            if (family_valid_words_[c] == 0u || family_valid_words_[c] > tile_words) {
                std::printf("[w4c0][FAIL] invalid family tile valid words case=%u\n", (unsigned)c);
                return false;
            }
    ...
    void build_mixed_bridge_payload(
        ...
        std::vector<aecct::u32_t>& family_d_tile_idx) const {
    ...
    bool run_positive_contract_case() {
        ...
        if (!softmax_mainline_taken_bridge || fallback_taken_bridge) {
            std::printf("[w4c0][FAIL] positive mixed bridge run did not stay on mainline\n");
            return false;
        }
        ...
        if ((uint32_t)family_visible_count.to_uint() != family_case_count_ ||
            ...
            std::printf("[w4c0][FAIL] family observability mismatch visible=%u owner=%u consumed=%u compare=%u mask=0x%X expected=0x%X\n",
        ...
        if (enable_single_bridge_) {
            if ((uint32_t)single_visible.to_uint() != 1u ||
                ...
                std::printf("[w4c0][FAIL] single observability mismatch visible=%u owner=%u consumed=%u compare=%u\n",
            ...
            // Topology with one d-tile cannot host disjoint single+family bridge selectors.
            if ((uint32_t)single_visible.to_uint() != 0u ||
                ...
                std::printf("[w4c0][FAIL] single observability must stay zero in family-only mode visible=%u owner=%u consumed=%u compare=%u\n",
                    (unsigned)(uint32_t)single_visible.to_uint(),
                    (unsigned)(uint32_t)single_owner_ok.to_uint(),
                    (unsigned)(uint32_t)single_consumed.to_uint(),
                    (unsigned)(uint32_t)single_compare_ok.to_uint());
        ...
        std::printf("W4C0_SOFTMAXOUT_CONTRACT_BRIDGE_VISIBLE PASS\n");
        std::printf("W4C0_SOFTMAXOUT_CONTRACT_OWNERSHIP_CHECK PASS\n");
        std::printf("W4C0_SOFTMAXOUT_CONTRACT_ANTI_FALLBACK PASS\n");
        ...
        if (!softmax_mainline_taken_legacy || fallback_taken_legacy) {
            std::printf("[w4c0][FAIL] legacy run did not stay on mainline\n");
            return false;
        }
        ...
        ATTN_W4C0_EXPECTED_COMPARE_LOOP: for (uint32_t i = 0u; i < d_model; ++i) {
            ...
            if (got_pre != exp || got_post != exp || got_out != exp) {
                std::printf("[w4c0][FAIL] expected compare mismatch idx=%u pre=0x%08X post=0x%08X out=0x%08X exp=0x%08X\n",
                    (unsigned)i,
                    (unsigned)got_pre,
                    (unsigned)got_post,
                    (unsigned)got_out,
            ...
            if (got_pre != legacy_pre || got_post != legacy_post || got_out != legacy_out) {
                std::printf("[w4c0][FAIL] legacy compare mismatch idx=%u bridge(pre/post/out)=(0x%08X/0x%08X/0x%08X) legacy=(0x%08X/0x%08X/0x%08X)\n",
                    (unsigned)i,
                    (unsigned)got_pre,
                    (unsigned)got_post,
                    (unsigned)got_out,
        ...
        std::printf("W4C0_SOFTMAXOUT_CONTRACT_EXPECTED_COMPARE PASS\n");
        std::printf("W4C0_SOFTMAXOUT_CONTRACT_LEGACY_COMPARE PASS\n");
        ...
        ATTN_W4C0_SPURIOUS_CHECK_LOOP: for (uint32_t i = 0u; i < (uint32_t)sram_bridge.size(); ++i) {
            ...
            if (before != after && allowed_write[i] == 0u) {
                std::printf("[w4c0][FAIL] spurious write addr=%u before=0x%08X after=0x%08X\n",
                    (unsigned)i,
                    (unsigned)before,
                    (unsigned)after);
                return false;
        ...
        std::printf("W4C0_SOFTMAXOUT_CONTRACT_NO_SPURIOUS_TOUCH PASS\n");
        return true;
    }
    ...
    bool run_negative_contract_mismatch_case() {
        ...
        if (softmax_mainline_taken || !fallback_taken) {
            std::printf("[w4c0][FAIL] mismatch mixed bridge did not reject as expected\n");
            return false;
        }
        if ((uint32_t)family_visible_count.to_uint() != 1u ||
            ...
            std::printf("[w4c0][FAIL] mismatch family flags mismatch visible=%u owner=%u consumed=%u compare=%u mask=0x%X\n",
                (unsigned)(uint32_t)family_visible_count.to_uint(),
                (unsigned)(uint32_t)family_owner_ok.to_uint(),
                (unsigned)(uint32_t)family_consumed_count.to_uint(),
                (unsigned)(uint32_t)family_compare_ok.to_uint(),
        ...
        if ((uint32_t)single_visible.to_uint() != 0u ||
            ...
            (uint32_t)single_consumed.to_uint() != 0u ||
            (uint32_t)single_compare_ok.to_uint() != 0u) {
            std::printf("[w4c0][FAIL] mismatch single flags expected zero visible=%u owner=%u consumed=%u compare=%u\n",
                (unsigned)(uint32_t)single_visible.to_uint(),
                (unsigned)(uint32_t)single_owner_ok.to_uint(),
                (unsigned)(uint32_t)single_consumed.to_uint(),
                (unsigned)(uint32_t)single_compare_ok.to_uint());
        ...
        ATTN_W4C0_REJECT_STALE_CHECK_LOOP: for (uint32_t i = 0u; i < (uint32_t)sram_negative.size(); ++i) {
            if ((uint32_t)sram_negative[i].to_uint() != (uint32_t)sram_bootstrap_[i].to_uint()) {
                std::printf("[w4c0][FAIL] reject path wrote stale state addr=%u\n", (unsigned)i);
                return false;
            }
        }
        std::printf("W4C0_SOFTMAXOUT_CONTRACT_MISMATCH_REJECT PASS\n");
        return true;
    }
};
...
CCS_MAIN(int argc, char** argv) {
    (void)argc;
    (void)argv;
    TbW4c0SoftmaxOutContractProbe tb;
