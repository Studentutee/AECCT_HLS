// W4-C5: SoftmaxOut WRITEBACK-path single selected consume bridge (local-only).
...
class TbW4c5SoftmaxOutWritebackSingleSelectedConsumeBridge {
public:
    int run_all() {
        if (!init_and_bootstrap()) { return 1; }
        if (!run_positive_consume_case()) { return 1; }
        if (!run_negative_mismatch_case()) { return 1; }
        std::printf("PASS: tb_w4c5_softmaxout_writeback_single_selected_consume_bridge\n");
        return 0;
    }
...
private:
    ...
    bool init_and_bootstrap() {
        ...
        if (token_count < 2u) {
            std::printf("[w4c5][FAIL] token_count < 2 cannot validate later-token writeback path\n");
            return false;
        }
        ...
        if (selected_valid_words_ == 0u || selected_valid_words_ > tile_words) {
            std::printf("[w4c5][FAIL] invalid selected tile valid words\n");
            return false;
        }

        // Force selected later token onto RENORM branch first.
        // C5 then validates WRITEBACK selected consume/compare on this descriptor.
    ...
    void build_selected_family_payload(
        ...
        ATTN_W4C5_BUILD_FAMILY_PAYLOAD_LOOP: for (uint32_t i = 0u; i < selected_valid_words_; ++i) {
            family_words_flat[i] = sram_bootstrap_[base + i];
        }
    }

    void build_selected_writeback_payload(
        std::vector<aecct::u32_t>& writeback_words,
        aecct::u32_t& writeback_base_word) const {
        ...
        const uint32_t selected_head_base = selected_head_ * d_head;
        const uint32_t selected_tile_offset = selected_d_tile_ * tile_words;
        writeback_base_word =
            (aecct::u32_t)(pre_row_base + selected_head_base + selected_tile_offset);
        writeback_words.assign(selected_valid_words_, (aecct::u32_t)0u);
        ATTN_W4C5_BUILD_WRITEBACK_PAYLOAD_LOOP: for (uint32_t i = 0u;
             i < selected_valid_words_; ++i) {
            writeback_words[i] =
                expected_out_[selected_head_base + selected_tile_offset + i];
        }
    }
    ...
    bool run_positive_consume_case() {
        ...
        std::vector<aecct::u32_t> writeback_selected_words;
        aecct::u32_t writeback_selected_base_word = (aecct::u32_t)0u;
        build_selected_writeback_payload(
            writeback_selected_words,
            writeback_selected_base_word);
        ...
        const bool softmax_mainline_taken_probe =
            aecct::run_p11af_layer0_top_managed_softmax_out(
                ...
                writeback_selected_base_word,
                writeback_selected_words.data(),
                (aecct::u32_t)selected_valid_words_,
        ...
        if (!softmax_mainline_taken_probe || fallback_taken_probe) {
            std::printf("[w4c5][FAIL] positive writeback consume run did not stay on mainline\n");
            return false;
        }
        ...
        if ((uint32_t)family_visible_count.to_uint() != 1u ||
            ...
            (uint32_t)writeback_selected_consumed_count.to_uint() != selected_valid_words_ ||
            (uint32_t)writeback_selected_owner_ok.to_uint() != 1u ||
            (uint32_t)writeback_selected_compare_ok.to_uint() != 1u) {
            std::printf("[w4c5][FAIL] writeback consume mismatch visible=%u owner=%u consumed=%u compare=%u mask=0x%X desc_visible=%u desc_mask=0x%X renorm_count=%u renorm_mask=0x%X wb_count=%u wb_mask=0x%X wb_touch=%u wb_expected_touch=%u wb_consume=%u wb_owner=%u wb_compare=%u\n",
        ...
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_SINGLE_SELECTED_VISIBLE PASS\n");
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_SINGLE_SELECTED_OWNERSHIP_CHECK PASS\n");
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_SINGLE_SELECTED_SELECTOR_CASE_MASK PASS\n");
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_SINGLE_SELECTED_WRITEBACK_PATH_VISIBILITY PASS\n");
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_SINGLE_SELECTED_WRITEBACK_TOUCH_COUNT_EXACT PASS\n");
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_SELECTED_CONSUME_EXACT PASS\n");
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_ANTI_FALLBACK PASS\n");
        ...
        if (!softmax_mainline_taken_legacy || fallback_taken_legacy) {
            std::printf("[w4c5][FAIL] legacy run did not stay on mainline\n");
            return false;
        }
        ...
        ATTN_W4C5_EXPECTED_COMPARE_LOOP: for (uint32_t i = 0u; i < d_model; ++i) {
            ...
            if (got_pre != exp || got_post != exp || got_out != exp) {
                std::printf("[w4c5][FAIL] expected compare mismatch idx=%u pre=0x%08X post=0x%08X out=0x%08X exp=0x%08X\n",
                    (unsigned)i,
                    (unsigned)got_pre,
                    (unsigned)got_post,
                    (unsigned)got_out,
            ...
            if (got_pre != legacy_pre || got_post != legacy_post || got_out != legacy_out) {
                std::printf("[w4c5][FAIL] legacy compare mismatch idx=%u bridge(pre/post/out)=(0x%08X/0x%08X/0x%08X) legacy=(0x%08X/0x%08X/0x%08X)\n",
                    (unsigned)i,
                    (unsigned)got_pre,
                    (unsigned)got_post,
                    (unsigned)got_out,
        ...
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_SINGLE_SELECTED_EXPECTED_COMPARE PASS\n");
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_SINGLE_SELECTED_LEGACY_COMPARE PASS\n");
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_EXPECTED_LEGACY_COMPARE PASS\n");
        ...
        ATTN_W4C5_SPURIOUS_CHECK_LOOP: for (uint32_t i = 0u; i < (uint32_t)sram_probe.size(); ++i) {
            ...
            if (before != after && allowed_write[i] == 0u) {
                std::printf("[w4c5][FAIL] spurious write addr=%u before=0x%08X after=0x%08X\n",
                    (unsigned)i,
                    (unsigned)before,
                    (unsigned)after);
                return false;
        ...
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_SINGLE_SELECTED_NO_SPURIOUS_TOUCH PASS\n");
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_NO_SPURIOUS_TOUCH PASS\n");
        return true;
    }
    ...
    bool run_negative_mismatch_case() {
        ...
        // Mismatch at selected writeback-consume payload should reject in C5 compare stage.
        writeback_selected_words[0u] =
            (aecct::u32_t)((uint32_t)writeback_selected_words[0u].to_uint() ^ 0x00000001u);
        ...
        if (softmax_mainline_taken || !fallback_taken) {
            std::printf("[w4c5][FAIL] writeback consume mismatch did not reject as expected\n");
            return false;
        }
        ...
        if ((uint32_t)family_visible_count.to_uint() != 1u ||
            ...
            std::printf("[w4c5][FAIL] mismatch flags mismatch visible=%u owner=%u consumed=%u compare=%u mask=0x%X desc_visible=%u desc_mask=0x%X renorm_count=%u renorm_mask=0x%X wb_count=%u wb_mask=0x%X wb_touch=%u wb_consume=%u wb_owner=%u wb_compare=%u\n",
                (unsigned)(uint32_t)family_visible_count.to_uint(),
                (unsigned)(uint32_t)family_owner_ok.to_uint(),
                (unsigned)(uint32_t)family_consumed_count.to_uint(),
                (unsigned)(uint32_t)family_compare_ok.to_uint(),
        ...
        ATTN_W4C5_REJECT_STALE_CHECK_LOOP: for (uint32_t i = 0u; i < (uint32_t)sram_negative.size(); ++i) {
            if ((uint32_t)sram_negative[i].to_uint() != (uint32_t)sram_bootstrap_[i].to_uint()) {
                std::printf("[w4c5][FAIL] reject path wrote stale state addr=%u\n", (unsigned)i);
                return false;
            }
        }
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_SINGLE_SELECTED_MISMATCH_REJECT PASS\n");
        std::printf("W4C5_SOFTMAXOUT_WRITEBACK_MISMATCH_REJECT PASS\n");
        return true;
    }
};
...
CCS_MAIN(int argc, char** argv) {
    (void)argc;
    (void)argv;
    TbW4c5SoftmaxOutWritebackSingleSelectedConsumeBridge tb;
