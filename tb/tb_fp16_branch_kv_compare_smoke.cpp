...
static bool build_matrix_payload_and_inv_fp32(
  ...
  if (!quant_linear_matrix_id_to_weight_id(matrix_id, payload_wid) ||
    ...
    std::fprintf(stderr,
                 "[fp16_kv][FAIL] matrix id mapping failed matrix=%u (%s)\n",
                 (unsigned)matrix_id,
  ...
  if (!fp16_branch_tb::build_weight_words16(payload_wid, payload_words16, payload_logical)) {
    std::fprintf(stderr,
                 "[fp16_kv][FAIL] payload word16 build failed matrix=%u (%s)\n",
                 (unsigned)matrix_id,
                 quant_linear_matrix_id_name(matrix_id));
    return false;
  }
  ...
  if (!fp16_branch_tb::build_weight_words16(inv_wid, inv_words16, inv_logical) || inv_words16.empty()) {
    std::fprintf(stderr,
                 "[fp16_kv][FAIL] inv_s_w word16 build failed matrix=%u (%s)\n",
                 (unsigned)matrix_id,
                 quant_linear_matrix_id_name(matrix_id));
    return false;
  }
...
static int run_one_sample(const uint32_t sample_idx) {
  ...
  if (!aecct::attn_phasea_top_managed_kv_mainline(
          ...
          sc,
          fallback_taken)) {
    std::fprintf(stderr, "[fp16_kv][FAIL] kv mainline returned false sample=%u\n", (unsigned)sample_idx);
    return 1;
  }
  if (fallback_taken) {
    std::fprintf(stderr, "[fp16_kv][FAIL] kv mainline took fallback sample=%u\n", (unsigned)sample_idx);
    return 1;
  }
  ...
  for (uint32_t t = 0u; t < token_count; ++t) {
    ...
    if (!aecct::ternary_live_l0_wk_materialize_row_kernel_split(
        ...
        !aecct::ternary_live_l0_wv_materialize_row_kernel_split(
            ...
            v_out_inv_sw_bits)) {
      std::fprintf(stderr,
                   "[fp16_kv][FAIL] reference kernel failed sample=%u token=%u\n",
                   (unsigned)sample_idx,
                   (unsigned)t);
      return 1;
    }
    ...
    for (uint32_t d = 0u; d < d_model; ++d) {
      ...
      if (got_k != exp_k || got_v != exp_v) {
        if (kv_mismatch == 0u) {
          std::fprintf(stderr,
                       "[fp16_kv][FAIL] first mismatch sample=%u token=%u d=%u got_k=0x%08X exp_k=0x%08X got_v=0x%08X exp_v=0x%08X\n",
                       (unsigned)sample_idx,
                       (unsigned)t,
                       (unsigned)d,
                       (unsigned)got_k,
  ...
  if (kv_mismatch != 0u) {
    std::fprintf(stderr,
                 "[fp16_kv][FAIL] total mismatches sample=%u count=%u\n",
                 (unsigned)sample_idx,
                 (unsigned)kv_mismatch);
    return 1;
  }

  std::printf("[fp16_kv][PASS] sample=%u tokens=%u d_model=%u kv_mismatch=0\n",
              (unsigned)sample_idx,
...
int main() {
  ...
  std::printf("PASS: tb_fp16_branch_kv_compare_smoke\n");
  return 0;
}
