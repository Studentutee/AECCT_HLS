...
int main() {
  ...
  std::vector<double> legacy_layer0_ln(
  ...
  io_legacy.debug.out_layer0_ln_out = legacy_layer0_ln.data();
  io_legacy.B = 1;
  io_legacy.N = VAR_N;
  ...
  std::vector<double> x_work_after_attn(
    static_cast<std::size_t>(TOKENS_T * D_MODEL),
    0.0);
  for (int t = 0; t < TOKENS_T; ++t) {
    for (int d = 0; d < D_MODEL; ++d) {
      x_work_after_attn[static_cast<std::size_t>(t * D_MODEL + d)] =
  ...
  if (!opt_model.layer0_ln_writeback_valid()) {
    std::printf("FAIL: layer0_ln_writeback_valid flag is false\n");
    return 4;
  }

  std::size_t ln_changed_count = 0;
  ...
  for (int t = 0; t < TOKENS_T; ++t) {
    for (int d = 0; d < D_MODEL; ++d) {
      ...
      const double opt_ln = static_cast<double>(opt_model.x_work(t, d).to_float());
      const double legacy_ln = legacy_layer0_ln[idx];
      if (std::fabs(opt_ln - x_work_after_attn[idx]) > 0.0) {
        ++ln_changed_count;
      }

      const double abs_diff = std::fabs(opt_ln - legacy_ln);
      ...
      if (abs_diff > kStageTol) {
        ...
        if (first_mismatch_t < 0) {
          first_mismatch_t = t;
          first_mismatch_d = d;
          first_mismatch_opt = opt_ln;
          first_mismatch_ref = legacy_ln;
        }
      }
    }
  }
  ...
  std::printf(
    "[opt_step5a][stage] ln_writeback_valid=1 ln_changed=%u/%u "
    "max_abs_diff_vs_legacy_ln=%.9g mismatch_gt_tol=%u tol=%.1e\n",
    static_cast<unsigned>(ln_changed_count),
  ...
  if (first_mismatch_t >= 0) {
    std::printf(
      "[opt_step5a][stage] first_mismatch token=%d dim=%d opt=%.9g ref=%.9g abs=%.9g\n",
      first_mismatch_t,
      first_mismatch_d,
      first_mismatch_opt,
      first_mismatch_ref,
  ...
  if (ln_changed_count == 0) {
    std::printf("FAIL: layer0 LN did not change X_WORK on any element\n");
    return 5;
  }
  if (mismatch_count != 0) {
    std::printf("FAIL: optimized layer0 LN mismatch against legacy layer0_ln_out boundary\n");
    return 6;
  }
  ...
  if (!opt_model.layer0_ln_writeback_valid()) {
    std::printf("FAIL: infer_step0 did not leave layer0_ln_writeback_valid asserted\n");
    return 7;
  }
  ...
  std::printf(
    "[opt_step5a][hybrid] x_pred_mismatch=%u/%u first_mismatch=%d "
  ...
  std::printf("PASS: tb_refmodel_optimized_step5a_layer0_ln_smoke\n");
  return 0;
}
