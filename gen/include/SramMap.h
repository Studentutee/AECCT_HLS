...
constexpr bool storage_word16_is_legacy_word_aligned(uint32_t words_word16) {
  return (words_word16 % SRAM_STORAGE_WORDS_PER_LEGACY_WORD) == 0u;
}

constexpr uint32_t storage_words_to_legacy_words_exact(uint32_t words_word16) {
  return words_word16 / SRAM_STORAGE_WORDS_PER_LEGACY_WORD;
}
...
// ------------------------------------------------------------
// Native fp16 baseline word16 map (clean storage-side authority)
// ------------------------------------------------------------
// This layout is the branch's intended storage-world view:
// - addresses are counted directly in 16-bit SRAM words
// - region placement is aligned to the native 8-word beat, not by first
//   laying everything out in legacy u32 space and multiplying by two
// - PARAM / W_REGION payload counts therefore reflect the real mixed storage
//   contract (fp16, ternary 2b, bitpack) instead of a whole-map x2 bridge
static const uint32_t FP16_BASELINE_ALIGN_WORD16 = SRAM_STORAGE_WORDS_PER_BEAT;

static const uint32_t FP16_BASELINE_X_WORK_BASE_WORD16 = 0u;
static const uint32_t FP16_BASELINE_X_WORK_WORDS_WORD16 =
    align_up_storage_words(storage_words_fp16(ELEMS_X), FP16_BASELINE_ALIGN_WORD16);

static const uint32_t FP16_BASELINE_SCR_K_BASE_WORD16 =
    align_up_storage_words(FP16_BASELINE_X_WORK_BASE_WORD16 + FP16_BASELINE_X_WORK_WORDS_WORD16,
                           FP16_BASELINE_ALIGN_WORD16);
static const uint32_t FP16_BASELINE_SCR_K_WORDS_WORD16 =
    align_up_storage_words(storage_words_fp16(ELEMS_X), FP16_BASELINE_ALIGN_WORD16);

static const uint32_t FP16_BASELINE_SCR_V_BASE_WORD16 =
    align_up_storage_words(FP16_BASELINE_SCR_K_BASE_WORD16 + FP16_BASELINE_SCR_K_WORDS_WORD16,
                           FP16_BASELINE_ALIGN_WORD16);
static const uint32_t FP16_BASELINE_SCR_V_WORDS_WORD16 =
    align_up_storage_words(storage_words_fp16(ELEMS_X), FP16_BASELINE_ALIGN_WORD16);

static const uint32_t FP16_BASELINE_FINAL_SCALAR_BUF_BASE_WORD16 =
    align_up_storage_words(FP16_BASELINE_SCR_V_BASE_WORD16 + FP16_BASELINE_SCR_V_WORDS_WORD16,
                           FP16_BASELINE_ALIGN_WORD16);
static const uint32_t FP16_BASELINE_FINAL_SCALAR_BUF_WORDS_WORD16 =
    align_up_storage_words(storage_words_fp16(N_NODES), FP16_BASELINE_ALIGN_WORD16);

static const uint32_t FP16_BASELINE_BIAS_BASE_WORD16 =
    align_up_storage_words(FP16_BASELINE_FINAL_SCALAR_BUF_BASE_WORD16 +
                               FP16_BASELINE_FINAL_SCALAR_BUF_WORDS_WORD16,
                           FP16_BASELINE_ALIGN_WORD16);
static const uint32_t FP16_BASELINE_BIAS_PAYLOAD_WORDS_WORD16 = FP16_BRANCH_TOTAL_BIAS_WORDS16;
static const uint32_t FP16_BASELINE_BIAS_WORDS_WORD16 =
    align_up_storage_words(FP16_BASELINE_BIAS_PAYLOAD_WORDS_WORD16, FP16_BASELINE_ALIGN_WORD16);

static const uint32_t FP16_BASELINE_WEIGHT_BASE_WORD16 =
    align_up_storage_words(FP16_BASELINE_BIAS_BASE_WORD16 + FP16_BASELINE_BIAS_WORDS_WORD16,
                           FP16_BASELINE_ALIGN_WORD16);
static const uint32_t FP16_BASELINE_WEIGHT_PAYLOAD_WORDS_WORD16 = FP16_BRANCH_TOTAL_WEIGHT_WORDS16;
static const uint32_t FP16_BASELINE_WEIGHT_WORDS_WORD16 =
    align_up_storage_words(FP16_BASELINE_WEIGHT_PAYLOAD_WORDS_WORD16, FP16_BASELINE_ALIGN_WORD16);

static const uint32_t FP16_BASELINE_W_REGION_BASE_WORD16 = FP16_BASELINE_BIAS_BASE_WORD16;
static const uint32_t FP16_BASELINE_W_REGION_WORDS_WORD16 =
    FP16_BASELINE_BIAS_WORDS_WORD16 + FP16_BASELINE_WEIGHT_WORDS_WORD16;
static const uint32_t FP16_BASELINE_W_REGION_PAYLOAD_WORDS_WORD16 =
    FP16_BASELINE_BIAS_PAYLOAD_WORDS_WORD16 + FP16_BASELINE_WEIGHT_PAYLOAD_WORDS_WORD16;

static const uint32_t FP16_BASELINE_PARAM_STREAM_DEFAULT_BASE_WORD16 = FP16_BASELINE_BIAS_BASE_WORD16;
static const uint32_t FP16_BASELINE_PARAM_STREAM_DEFAULT_WORDS_WORD16 = FP16_BRANCH_TOTAL_PARAM_WORDS16;

static const uint32_t FP16_BASELINE_IO_REGION_BASE_WORD16 =
    FP16_BASELINE_WEIGHT_BASE_WORD16 + FP16_BASELINE_WEIGHT_WORDS_WORD16;
static const uint32_t FP16_BASELINE_IO_REGION_WORDS_WORD16 = 0u;

static const uint32_t FP16_BASELINE_STORAGE_WORDS_MIN_REQUIRED = FP16_BASELINE_IO_REGION_BASE_WORD16;
static const uint32_t FP16_BASELINE_BACKUP_RUNTIME_SCRATCH_BASE_WORD16 =
    align_up_storage_words(FP16_BASELINE_STORAGE_WORDS_MIN_REQUIRED, FP16_BASELINE_ALIGN_WORD16);
static const uint32_t FP16_BASELINE_BACKUP_RUNTIME_SCRATCH_WORDS_WORD16 =
    align_up_storage_words(legacy_words_to_storage_words(BACKUP_RUNTIME_SCRATCH_WORDS),
                           FP16_BASELINE_ALIGN_WORD16);
static const uint32_t FP16_BASELINE_STORAGE_WORDS_TOTAL =
    FP16_BASELINE_BACKUP_RUNTIME_SCRATCH_BASE_WORD16 + FP16_BASELINE_BACKUP_RUNTIME_SCRATCH_WORDS_WORD16;

enum Fp16BaselineSectionId : uint8_t {
  FP16_BASE_SEC_X_WORK = 0,
  FP16_BASE_SEC_SCR_K = 1,
  FP16_BASE_SEC_SCR_V = 2,
  FP16_BASE_SEC_FINAL_SCALAR_BUF = 3,
  FP16_BASE_SEC_BIAS = 4,
  FP16_BASE_SEC_WEIGHT = 5,
  FP16_BASE_SEC_W_REGION = 6,
  FP16_BASE_SEC_PARAM_STREAM_DEFAULT = 7,
  FP16_BASE_SEC_BACKUP_RUNTIME_SCRATCH = 8,
  FP16_BASE_SEC_INVALID = 255
};

struct Fp16BaselineSectionDesc {
  uint8_t id;
  uint8_t storage_class;
  uint8_t alias_group;
  uint8_t payload_class;
...
static constexpr Fp16BaselineSectionDesc kFp16BaselineSectionTable[] = {
  { FP16_BASE_SEC_X_WORK, CLASS_X_WORK, ALIAS_X_WORK, PAYLOAD_FP16_TENSOR,
    FP16_BASELINE_X_WORK_BASE_WORD16, FP16_BASELINE_X_WORK_WORDS_WORD16,
    storage_words_fp16(ELEMS_X),
    SECTION_FLAG_READ_MEM_VISIBLE | SECTION_FLAG_COMPARE_CRITICAL },
  { FP16_BASE_SEC_SCR_K, CLASS_SCRATCH, ALIAS_SCR_K, PAYLOAD_FP16_TENSOR,
    FP16_BASELINE_SCR_K_BASE_WORD16, FP16_BASELINE_SCR_K_WORDS_WORD16,
    storage_words_fp16(ELEMS_X),
    SECTION_FLAG_READ_MEM_VISIBLE | SECTION_FLAG_COMPARE_CRITICAL },
  { FP16_BASE_SEC_SCR_V, CLASS_SCRATCH, ALIAS_SCR_V, PAYLOAD_FP16_TENSOR,
    FP16_BASELINE_SCR_V_BASE_WORD16, FP16_BASELINE_SCR_V_WORDS_WORD16,
    storage_words_fp16(ELEMS_X),
    SECTION_FLAG_READ_MEM_VISIBLE | SECTION_FLAG_COMPARE_CRITICAL },
  { FP16_BASE_SEC_FINAL_SCALAR_BUF, CLASS_SCRATCH, ALIAS_FINAL_SCALAR, PAYLOAD_FP16_VECTOR,
    FP16_BASELINE_FINAL_SCALAR_BUF_BASE_WORD16, FP16_BASELINE_FINAL_SCALAR_BUF_WORDS_WORD16,
    storage_words_fp16(N_NODES),
    SECTION_FLAG_READ_MEM_VISIBLE | SECTION_FLAG_COMPARE_CRITICAL | SECTION_FLAG_PADDING_PRESENT },
  { FP16_BASE_SEC_BIAS, CLASS_W_REGION, ALIAS_W_PERSIST, PAYLOAD_MIXED_PERSIST,
    FP16_BASELINE_BIAS_BASE_WORD16, FP16_BASELINE_BIAS_WORDS_WORD16,
    FP16_BASELINE_BIAS_PAYLOAD_WORDS_WORD16,
    SECTION_FLAG_LOAD_W_CRITICAL | SECTION_FLAG_READ_MEM_VISIBLE | SECTION_FLAG_PADDING_PRESENT },
  { FP16_BASE_SEC_WEIGHT, CLASS_W_REGION, ALIAS_W_PERSIST, PAYLOAD_MIXED_PERSIST,
    FP16_BASELINE_WEIGHT_BASE_WORD16, FP16_BASELINE_WEIGHT_WORDS_WORD16,
    FP16_BASELINE_WEIGHT_PAYLOAD_WORDS_WORD16,
    SECTION_FLAG_LOAD_W_CRITICAL | SECTION_FLAG_READ_MEM_VISIBLE | SECTION_FLAG_PADDING_PRESENT },
  { FP16_BASE_SEC_W_REGION, CLASS_W_REGION, ALIAS_W_PERSIST, PAYLOAD_MIXED_PERSIST,
    FP16_BASELINE_W_REGION_BASE_WORD16, FP16_BASELINE_W_REGION_WORDS_WORD16,
    FP16_BASELINE_W_REGION_PAYLOAD_WORDS_WORD16,
    SECTION_FLAG_LOAD_W_CRITICAL | SECTION_FLAG_READ_MEM_VISIBLE | SECTION_FLAG_PADDING_PRESENT },
  { FP16_BASE_SEC_PARAM_STREAM_DEFAULT, CLASS_W_REGION, ALIAS_W_PERSIST, PAYLOAD_PARAM_STREAM,
    FP16_BASELINE_PARAM_STREAM_DEFAULT_BASE_WORD16, FP16_BASELINE_PARAM_STREAM_DEFAULT_WORDS_WORD16,
    FP16_BASELINE_PARAM_STREAM_DEFAULT_WORDS_WORD16,
    SECTION_FLAG_LOAD_W_CRITICAL | SECTION_FLAG_READ_MEM_VISIBLE },
  { FP16_BASE_SEC_BACKUP_RUNTIME_SCRATCH, CLASS_SCRATCH, ALIAS_LOCAL_ONLY, PAYLOAD_RUNTIME_SCRATCH,
    FP16_BASELINE_BACKUP_RUNTIME_SCRATCH_BASE_WORD16, FP16_BASELINE_BACKUP_RUNTIME_SCRATCH_WORDS_WORD16,
    FP16_BASELINE_BACKUP_RUNTIME_SCRATCH_WORDS_WORD16,
    SECTION_FLAG_READ_MEM_VISIBLE | SECTION_FLAG_COMPARE_CRITICAL }
};

static const uint32_t FP16_BASELINE_SECTION_COUNT =
    (uint32_t)(sizeof(kFp16BaselineSectionTable) / sizeof(kFp16BaselineSectionTable[0]));

static inline const Fp16BaselineSectionDesc& fp16_baseline_section_desc(const Fp16BaselineSectionId id) {
  const uint32_t idx = (uint32_t)id;
  if (idx < FP16_BASELINE_SECTION_COUNT) {
    return kFp16BaselineSectionTable[idx];
  }
  return kFp16BaselineSectionTable[0];
}

static inline constexpr bool fp16_baseline_default_param_stream_fits_w_region() {
  const unsigned long long begin = (unsigned long long)FP16_BASELINE_PARAM_STREAM_DEFAULT_BASE_WORD16;
  const unsigned long long end_excl = begin + (unsigned long long)FP16_BASELINE_PARAM_STREAM_DEFAULT_WORDS_WORD16;
  const unsigned long long region_begin = (unsigned long long)FP16_BASELINE_W_REGION_BASE_WORD16;
  const unsigned long long region_end_excl =
      region_begin + (unsigned long long)FP16_BASELINE_W_REGION_WORDS_WORD16;
  return (begin >= region_begin) && (end_excl <= region_end_excl);
}

static_assert(fp16_baseline_default_param_stream_fits_w_region(),
              "fp16 baseline PARAM stream must fit inside native word16 W_REGION.");
static_assert(FP16_BASELINE_PARAM_STREAM_DEFAULT_WORDS_WORD16 == FP16_BRANCH_TOTAL_PARAM_WORDS16,
              "fp16 baseline PARAM words16 must match mixed branch payload contract.");
static_assert(FP16_BASELINE_STORAGE_WORDS_MIN_REQUIRED <= SRAM_STORAGE_WORDS_MIN_REQUIRED,
              "native fp16 word16 baseline should not exceed bridged x2 storage minimum.");
