# TOP MANAGED SRAM W4-M2 COMPLETION (2026-03-30)

## 1. Summary
- Completed W4-M2 bounded micro-cut on SoftmaxOut phase entry:
  - added caller-fed V-tile probe passthrough at Top helper entry,
  - added phase-entry visibility/ownership/compare probe checks in SoftmaxOut mainline entry.
- Kept inner online softmax/reduction and writeback loops unchanged.
...
## 6. Residual risks
- SoftmaxOut inner online accumulation/writeback loops remain SRAM-centric.
- W4-M2 is probe visibility/ownership anchoring only, not full Wave4 payload migration.
- W4-M3 remains feasibility/refinement only in this run.

## 7. Recommended next step
- Execute W4-M3 bounded phase-entry probe (Phase-A Q x-row descriptor) with the same probe visibility/ownership/no-spurious validation pattern.
