...
## Suggested 10-Minute Review Order (G5 FFN W1 Fallback Policy Latest)
1. Open `src/blocks/FFNLayer0.h` (2 min)
   - Confirm `FFN_POLICY_REQUIRE_W1_TOPFED` gate and descriptor-ready reject path before W1 compute loop.
   - Confirm `topfed_x_words_valid_override` and `x_idx < topfed_x_valid` anchor.
2. Open `src/blocks/TransformerLayer.h` (1 min)
   - Confirm W1 stage call passes `(u32_t)FFN_POLICY_REQUIRE_W1_TOPFED`.
   - Confirm caller passes `(u32_t)ffn_x_words` as x descriptor-ready valid override.
3. Open `build/p11g5/ffn_w1_fallback_policy/run.log` (1 min)
   - Confirm:
...
4. Open `build/top_managed_sram_guard/check_top_managed_sram_boundary_regression.log` (1 min)
   - Confirm `guard: G5 FFN W1 fallback policy strict top-fed gating anchors OK`.
5. Open regression logs (2 min)
   - `build/p11g5/ffn_fallback_policy/run.log`
   - `build/p11g5/ffn_closure_campaign/run.log`
   - `build/p11g5/wave3_ffn_payload_migration/run.log`
   - `build/p11g5/wave35_ffn_w1_weight_migration/run.log`
   - Confirm all PASS banners remain stable.
6. Open `build/p11ah/full_loop/run.log` and `build/p11aj/p11aj/run.log` (1 min)
   - Confirm mainline/provenance PASS.
7. Open `docs/handoff/TOP_MANAGED_SRAM_G5_FFN_W1_FALLBACK_POLICY_20260330.md` (1 min)
8. Open `docs/handoff/TOP_MANAGED_SRAM_G5_FFN_W1_FALLBACK_EVIDENCE_INDEX_20260330.md` (1 min)
9. Open `build/evidence/g5_ffn_w1_fallback_policy_20260330/evidence_manifest.txt` (1 min)

## Latest artifact additions (G5 FFN W1 fallback policy)
...
## Suggested 10-Minute Review Order (W4-M3 Phase-A Q Probe Latest)
1. Open `src/blocks/AttnPhaseATopManagedQ.h` (2 min)
   - Confirm phase-entry probe anchors:
     - `phase_entry_probe_enabled`
     - `ATTN_P11AD_PHASE_ENTRY_PROBE_COL_LOOP`
     - ownership/compare observability outputs.
2. Open `src/Top.h` (1 min)
   - Confirm `run_p11ad_layer0_top_managed_q(...)` includes probe passthrough args.
3. Open `build/p11w4m3/phasea_q_phase_entry_probe/run.log` (1 min)
   - Confirm:
...
4. Open `build/top_managed_sram_guard/check_top_managed_sram_boundary_regression.log` (1 min)
   - Confirm `guard: W4-M3 Phase-A Q phase-entry caller-fed x-row probe anchors OK`.
5. Open retained regression logs (2 min)
   - `build/p11w4m2/softmaxout_phase_entry_probe/run.log`
   - `build/p11g6/ffn_w1_bias_descriptor/run.log`
   - `build/p11g6/ffn_fallback_observability/run.log`
   ...
   - `build/p11g5/ffn_fallback_policy/run.log`
6. Open `build/p11ah/full_loop/run.log` and `build/p11aj/p11aj/run.log` (1 min)
7. Open `docs/handoff/TOP_MANAGED_SRAM_W4M3_PHASEAQ_PROBE_20260330.md` and `docs/handoff/TOP_MANAGED_SRAM_W4M3_COMPLETION_20260330.md` (1 min)
8. Open `docs/handoff/TOP_MANAGED_SRAM_W4M3_KV_FEASIBILITY_20260330.md` (1 min)
9. Open `build/evidence/w4_phasea_q_probe_campaign_20260330/evidence_manifest.txt` (1 min)

## Latest artifact additions (W4-M3 campaign)
...
## Suggested 10-Minute Review Order (W4-B2 QkScore Tile Bridge Latest)
1. Open `src/blocks/AttnPhaseBTopManagedQkScore.h` (2 min)
- confirm W4-B2 bridge fields: `score_tile_bridge_*`
- confirm bounded bridge consume gate at `ATTN_P11AE_KEY_TOKEN_LOOP`.
2. Open `src/Top.h` (1 min)
- confirm `run_p11ae_layer0_top_managed_qk_score(...)` passthrough includes W4-B2 bridge args.
3. Open `build/p11w4b2/qkscore_tile_bridge/run.log` (1 min)
- confirm:
...
4. Open `build/top_managed_sram_guard/check_top_managed_sram_boundary_regression.log` (1 min)
- confirm `guard: W4-B2 QkScore bounded score-tile bridge anchors OK`.
5. Open retained Wave4 runners (1 min)
- `build/p11w4b1/phaseb_tile_bridge/run.log`
- `build/p11w4m3/kv_phase_entry_probe/run.log`
- `build/p11w4m3/phasea_q_phase_entry_probe/run.log`
...
## Latest artifact additions (W4-B2 campaign)
- `src/blocks/AttnPhaseBTopManagedQkScore.h`
- `src/Top.h`
