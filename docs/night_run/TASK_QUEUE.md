...
| NR-CHECK-DESIGN-PURITY-ACTIVE-009 | ready | checker | - | checker.design_purity | true | Active dispatch precheck for the next partial-bucket audit round after bucket-009 shrink. | Task log must contain `PASS: check_design_purity`; exit code = 0. |
| NR-RUNNER-NEXT-PARTIAL-BUCKET-AUDIT-010 | queued | runner | NR-CHECK-DESIGN-PURITY-ACTIVE-009 | runner.local.p11aj | true | Select next safest partial bucket after payload-enabled `11101` OUT-only shrink. | Must emit case-specific PASS/blocked banner and preserve already-converged bucket behavior. |
