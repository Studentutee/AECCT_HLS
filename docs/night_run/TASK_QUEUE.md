...
| NR-CHECK-DESIGN-PURITY-ACTIVE-005 | ready | checker | - | checker.design_purity | true | Active dispatch precheck for the next partial-bucket audit round after bucket-005 shrink. | Task log must contain `PASS: check_design_purity`; exit code = 0. |
| NR-RUNNER-NEXT-PARTIAL-BUCKET-AUDIT-006 | queued | runner | NR-CHECK-DESIGN-PURITY-ACTIVE-005 | runner.local.p11aj | true | Select next safest partial bucket after kv-ready/q-not-prebuilt score-ready OUT-stage shrink. | Must emit case-specific PASS/blocked banner and preserve already-converged bucket behavior. |
