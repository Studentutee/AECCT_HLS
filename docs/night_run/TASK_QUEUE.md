...
| NR-CHECK-DESIGN-PURITY-ACTIVE-007 | ready | checker | - | checker.design_purity | true | Active dispatch precheck for the next partial-bucket audit round after bucket-007 shrink. | Task log must contain `PASS: check_design_purity`; exit code = 0. |
| NR-RUNNER-NEXT-PARTIAL-BUCKET-AUDIT-008 | queued | runner | NR-CHECK-DESIGN-PURITY-ACTIVE-007 | runner.local.p11aj | true | Select next safest partial bucket after q/kv-not-prebuilt score-ready OUT-stage shrink. | Must emit case-specific PASS/blocked banner and preserve already-converged bucket behavior. |
