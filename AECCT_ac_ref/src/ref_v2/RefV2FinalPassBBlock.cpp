...
bool RefV2FinalPassBBlock::run(ac_channel<RefV2FinalScalarTokenPayload>& in_scalar_ch,
  ...
  ref_fp32_t final_scalar_buf[REFV2_TOKENS_T];
  bool token_seen[REFV2_TOKENS_T];
  bool header_init = false;
  RefV2AttentionPayloadHeader header_ref;

  REFV2_FINALB_TOKEN_INIT_LOOP: for (int token = 0; token < REFV2_TOKENS_T; ++token) {
    final_scalar_buf[token] = ref_fp32_t(0.0f);
    token_seen[token] = false;
  }

  REFV2_FINALB_TOKEN_READ_LOOP: for (int token_rx = 0; token_rx < REFV2_TOKENS_T; ++token_rx) {
    ...
    final_scalar_buf[token] = scalar_payload.scalar;
  }
  ...
  REFV2_FINALB_LOGITS_LOOP: for (int n = 0; n < REFV2_VAR_N; ++n) {
    ref_fp32_t acc(static_cast<float>(w_out_fc_bias[n]));

    REFV2_FINALB_TOKEN_REDUCE_LOOP: for (int token = 0; token < REFV2_TOKENS_T; ++token) {
      const ref_fp32_t w_nt(static_cast<float>(w_out_fc_weight[n * REFV2_TOKENS_T + token]));
      acc += (w_nt * final_scalar_buf[token]);
    }
