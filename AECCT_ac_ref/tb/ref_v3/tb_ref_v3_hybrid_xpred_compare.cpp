...
#error "tb_ref_v3_hybrid_xpred_compare is host-only."
#endif
...
struct PrefixArtifacts {
  bool run_ok = false;
  bool midnorm_ok = false;
  bool endnorm_ok = false;
  bool finalpassa_ok = false;
  double midnorm_xwork[kTokens * kDim]{};
...
struct ScoreboardResult {
  int pattern_idx = -1;
  SplicePointId splice_point = MIDNORM_OUT;
...
static bool run_dut_prefix_artifacts(int pattern_idx, PrefixArtifacts* out) {
  ...
  const aecct_ref::ref_v3::RefV3AttentionInputPayload midnorm_payload = ch_xwork1_side.read();
  if (!copy_xwork_payload_to_matrix(midnorm_payload, aecct_ref::ref_v3::REFV3_LAYER1_ID, out->midnorm_xwork)) {
    return false;
  }
  ch_xwork1_to_l1_attn.write(midnorm_payload);
  out->midnorm_ok = true;
  ...
  if (!copy_token_stream_to_matrix(ch_endnorm_to_finala, aecct_ref::ref_v3::REFV3_LAYER1_ID, out->endnorm_xwork)) {
    return false;
  }
  out->endnorm_ok = true;
...
static bool run_ref_prefix_artifacts(int pattern_idx, PrefixArtifacts* out) {
  ...
  REFV3_HYBRID_REF_MIDNORM_COPY_TOKEN_LOOP: for (int token = 0; token < kTokens; ++token) {
    REFV3_HYBRID_REF_MIDNORM_COPY_DIM_LOOP: for (int dim = 0; dim < kDim; ++dim) {
      const int idx = (token * kDim) + dim;
      out->midnorm_xwork[idx] = static_cast<double>(ref_model.x_work(token, dim).to_float());
    }
  }
  out->midnorm_ok = true;

  if (!ref_model.run_step0_layer1_attn_input_handoff() ||
...
static bool run_ref_suffix_from_midnorm(
  const double midnorm_xwork[kTokens * kDim],
  const double input_y_row[kVarN],
  aecct_ref::bit1_t out_xpred[kVarN]) {
  aecct_ref::RefRunConfig run_cfg = aecct_ref::make_fp32_baseline_run_config();
  aecct_ref::RefModelOptimized ref_model;
  ...
  // Host-only splice adapter: overwrite reference X_WORK at midnorm boundary.
  REFV3_HYBRID_REF_SPLICE_MIDNORM_COPY_TOKEN_LOOP: for (int token = 0; token < kTokens; ++token) {
    REFV3_HYBRID_REF_SPLICE_MIDNORM_COPY_DIM_LOOP: for (int dim = 0; dim < kDim; ++dim) {
      const int idx = (token * kDim) + dim;
      ref_model.storage_fp32_.x_work[token][dim] = ac_ieee_float<binary32>(
        static_cast<float>(midnorm_xwork[idx]));
    }
  }
  ...
  ref_model.mid_norm_writeback_valid_ = true;
  ...
  REFV3_HYBRID_REF_SPLICE_MIDNORM_FINALA_COPY_LOOP: for (int token = 0; token < kTokens; ++token) {
...
static bool run_dut_suffix_from_midnorm(
  const double midnorm_xwork[kTokens * kDim],
  int pattern_idx,
  aecct_ref::bit1_t out_xpred[kVarN]) {
  aecct_ref::RefRunConfig run_cfg = aecct_ref::make_fp32_baseline_run_config();
  ...
  ac_channel<aecct_ref::ref_v3::RefV3AttentionTokenVectorPayload> ch_midnorm_token_in;
  ac_channel<aecct_ref::ref_v3::RefV3AttentionInputPayload> ch_midnorm_xwork_in;
  ac_channel<aecct_ref::ref_v3::RefV3AttentionTokenVectorPayload> ch_l1_attn_to_ffn;
  ac_channel<aecct_ref::ref_v3::RefV3AttentionTokenVectorPayload> ch_l1_ffn_to_endnorm;
  ac_channel<aecct_ref::ref_v3::RefV3AttentionTokenVectorPayload> ch_endnorm_to_finala;
  ...
  aecct_ref::ref_v3::RefV3AttentionInputPayload midnorm_payload;
  if (!build_xwork_payload_from_matrix(
        aecct_ref::ref_v3::REFV3_LAYER1_ID,
        midnorm_xwork,
        &midnorm_payload)) {
    return false;
  ...
  ch_midnorm_xwork_in.write(midnorm_payload);

  if (!layer1_attn_ln_path.run(run_cfg, ch_midnorm_token_in, ch_midnorm_xwork_in, ch_l1_attn_to_ffn)) {
    return false;
  }
  if (!layer1_ffn_path.run(ch_l1_attn_to_ffn, ch_l1_ffn_to_endnorm)) {
    return false;
  ...
  REFV3_HYBRID_DUT_SUFFIX_MIDNORM_XPRED_COPY_LOOP: for (int n = 0; n < kVarN; ++n) {
...
static void score_against_trace(
  ...
  out->all_zero = (out->dut_one_count == 0) ? 1 : 0;
...
static bool run_hybrid_case(
  ...
  bool exec_ok = false;
  if (splice_point == MIDNORM_OUT) {
    if (mode == DUT_PREFIX_REF_SUFFIX) {
      if (!dut_prefix.midnorm_ok) {
        out->run_ok = false;
        return true;
      }
      exec_ok = run_ref_suffix_from_midnorm(dut_prefix.midnorm_xwork, input_y_row, hybrid_xpred);
    } else {
      if (!ref_prefix.midnorm_ok) {
        out->run_ok = false;
        return true;
      }
      exec_ok = run_dut_suffix_from_midnorm(ref_prefix.midnorm_xwork, pattern_idx, hybrid_xpred);
    }
...
static void print_case_line(const ScoreboardResult& result) {
  std::printf(
    "[ref_v3_hybrid_xpred_compare] pattern_idx=%d splice_point=%s mode=%s "
    "dut_zero_count=%d dut_one_count=%d all_zero=%d trace_zero_count=%d trace_one_count=%d "
    "winner_by_zero_count=%s match_trace_count=%d mismatch_trace_count=%d "
    "trace_non_binary_anomaly_count=%d result=%s\n",
    result.pattern_idx,
    splice_point_name(result.splice_point),
    mode_name(result.mode),
    result.run_ok ? result.dut_zero_count : -1,
    result.run_ok ? result.dut_one_count : -1,
    result.run_ok ? result.all_zero : 0,
...
int main() {
  ...
  REFV3_HYBRID_PATTERN_LOOP: for (int i = 0; i < static_cast<int>(kPatternIndices.size()); ++i) {
    ...
    REFV3_HYBRID_SPLICE_LOOP: for (int splice = static_cast<int>(MIDNORM_OUT);
  ...
  REFV3_HYBRID_SUMMARY_SPLICE_LOOP: for (int splice = static_cast<int>(MIDNORM_OUT);
       splice <= static_cast<int>(FINALPASSA_OUT);
       ++splice) {
    REFV3_HYBRID_SUMMARY_MODE_LOOP: for (int mode = static_cast<int>(DUT_PREFIX_REF_SUFFIX);
      ...
      std::printf(
        "[ref_v3_hybrid_xpred_compare_summary] splice_point=%s mode=%s "
  ...
  std::printf(
    "[ref_v3_hybrid_xpred_compare_total] total_patterns=%d total_cases=%d run_ok=%d run_fail=%d\n",
  ...
  if (run_fail != 0) {
    std::printf("FAIL: tb_ref_v3_hybrid_xpred_compare\n");
    return 2;
  }

  std::printf("PASS: tb_ref_v3_hybrid_xpred_compare\n");
  return 0;
}
